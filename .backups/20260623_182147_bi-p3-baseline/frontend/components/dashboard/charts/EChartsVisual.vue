  ...
  if (aggFn) {
    ...
    data = Array.from(grouped.entries())
      .map(([name, vals], i: number) => ({
        ...
        itemStyle: { color: resolveColorInput(colors[i % colors.length]), opacity: crossOpacity(name) }
      }))
    ...
    data = rows
      .map((r: any, i: number) => ({
        ...
        itemStyle: { color: resolveColorInput(colors[i % colors.length]), opacity: crossOpacity(r[keyLower]) }
      }))
