...
# Terminal TestResult statuses (the deterministic matcher writes pass/fail).
_TERMINAL = {"pass", "fail", "error", "stopped", "success"}
...
async def _await_run_terminal(
    ...
    *,
    ...
    """Poll a TestRun's TestResult rows until all reach a terminal state (or
    timeout), then return them. Guarded -> returns whatever rows we have."""
    ...
    while True:
        try:
            ...
            last_rows = list(res.scalars().all())
        except Exception as e:
            logger.warning("skill optimizer rollout poll failed: %s", e)
            return last_rows

        if last_rows and all(
            getattr(r, "status", "") in _TERMINAL for r in last_rows
        ):
            return last_rows
        if time.monotonic() >= deadline:
            logger.warning(
                "skill optimizer rollout timed out (run=%s); reading partial results",
                run_id,
            )
            return last_rows
...
async def _rollout(
    ...
    *,
    ...
    try:
        ...
        if run is None:
            return 0.0, []
        rows = await _await_run_terminal(db, str(run.id))
