<template>
  <div v-if="pageLoaded" class="h-screen w-full grid lg:grid-cols-[minmax(0,0.95fr)_1.05fr] bg-white">
    ...
    <div class="hidden lg:flex items-center justify-center p-8" style="background:radial-gradient(120% 120% at 80% 0%,#fbf3ee 0%,#faf8f5 42%,#f7f5f1 100%)">
      <div class="preview-card a-float w-full max-w-[600px] bg-white border border-[#eae6df] rounded-[20px] overflow-hidden shadow-[0_24px_60px_-28px_rgba(60,40,20,.25),0_2px_8px_rgba(0,0,0,.03)]">
        <!-- panel top bar -->
        <div class="flex items-center gap-2.5 px-4 py-3 border-b border-[#f1ede6]">
          ...
          <span class="ml-auto inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-600"><i class="live-dot w-1.5 h-1.5 rounded-full bg-emerald-500 block"></i>Live</span>
        </div>

        <div class="p-5">
          <!-- question -->
          <div class="flex justify-end mb-3.5">
            <div class="a-bubble bg-[#c2683f] text-white text-[13px] px-3.5 py-2.5 rounded-2xl rounded-br-[5px] shadow-[0_6px_16px_-6px_rgba(194,104,63,.5)]">Top 5 artists by revenue this quarter?</div>
          </div>

          <!-- answer -->
          <div class="a-answer flex gap-3 mb-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="grid grid-cols-3 gap-2.5 mb-3.5">
                <div class="a-kpi bg-[#faf8f5] border border-[#f1ede6] rounded-xl px-3 py-2.5">
              ...
              <!-- bar chart -->
              <div class="a-chart bg-[#faf8f5] border border-[#f1ede6] rounded-xl px-3.5 pt-3 pb-2">
                ...
                <div class="flex items-end gap-3.5 h-24 pt-1">
                  <div v-for="(b, i) in bars" :key="b.l" class="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                    <div class="a-bar w-full max-w-[34px] rounded-t-md bg-gradient-to-b from-[#e2a883] to-[#c2683f]" :style="{ height: b.h, animationDelay: (0.62 + i * 0.1) + 's', '--bd': (1.4 + i * 0.18) + 's' }"></div>
          ...
          <div class="grid grid-cols-4 gap-2.5">
            <div v-for="(tile, i) in tiles" :key="tile.l"
              class="a-tile bg-white border rounded-xl px-2.5 py-2.5 flex flex-col gap-2 transition-colors"
          ...
          <!-- composer -->
          <div class="a-composer mt-4 flex items-center gap-3 bg-white border border-[#eae6df] rounded-[13px] px-3 py-3 shadow-[0_4px_14px_-10px_rgba(0,0,0,.2)]">
            ...
            <div class="flex-1 leading-tight">
              <div class="text-[12.5px] text-[#191613] font-medium"><span class="caret">Ask anything about your data…</span></div>
  ...
  .a-bubble   { animation: pop  .5s cubic-bezier(.21,1,.32,1) .15s both; }
  .a-answer   { animation: rise .6s cubic-bezier(.21,1,.32,1) .35s both; }
  .a-kpi      { animation: pop  .5s cubic-bezier(.21,1,.32,1) both; }
  .a-kpi:nth-child(1) { animation-delay: .5s; }
  .a-kpi:nth-child(2) { animation-delay: .58s; }
  .a-kpi:nth-child(3) { animation-delay: .66s; }
  .a-chart    { animation: rise .6s cubic-bezier(.21,1,.32,1) .55s both; }
  .a-bar      { transform-origin: bottom; animation: grow .7s cubic-bezier(.21,1,.32,1) both; }
  .a-tile     { animation: pop  .45s cubic-bezier(.21,1,.32,1) both; }
  .a-composer { animation: rise .6s cubic-bezier(.21,1,.32,1) 1.35s both; }

  /* ===== ambient loops ===== */
  /* whole card breathing float */
  @keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
  .a-float { animation: float 7s ease-in-out 1.6s infinite; }
  ...
  /* bars gently breathe after they grow (scaleY around its own --bd height) */
  @keyframes barbreathe { 0%,100% { transform: scaleY(1); } 50% { transform: scaleY(.965); } }
  .a-bar { animation: grow .7s cubic-bezier(.21,1,.32,1) both, barbreathe var(--bd,1.6s) ease-in-out 1.6s infinite; }

  /* typing caret on composer placeholder */
  .caret::after { content: ''; display: inline-block; width: 1.5px; height: 1em; margin-left: 1px; background: #c2683f; vertical-align: -2px; animation: blink 1.05s step-end infinite; }
  @keyframes blink { 50% { opacity: 0; } }

  @media (prefers-reduced-motion: reduce) {
    .a-bubble, .a-answer, .a-kpi, .a-chart, .a-bar, .a-tile, .a-composer, .a-float, .live-dot { animation: none !important; }
    .a-bar { transform: scaleY(1) !important; }
