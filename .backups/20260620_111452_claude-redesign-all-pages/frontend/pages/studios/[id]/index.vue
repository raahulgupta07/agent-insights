<template>
    <div class="flex h-full text-sm w-full">
            ...
            <template v-else-if="studio">
                ...
                <aside class="w-60 shrink-0 bg-gray-50 border-e border-gray-200 flex flex-col overflow-y-auto h-full">
                    <!-- studio header -->
                    <div class="px-3 pt-4 pb-3 border-b border-gray-200">
                        <button class="text-xs text-gray-400 hover:text-gray-600 mb-2 inline-flex items-center gap-1" @click="router.push('/studios')">
                            <UIcon name="i-heroicons-arrow-left" class="w-3.5 h-3.5" /> {{ $t('studio.backToStudios') }}
                        </button>
                        <button
                            type="button"
                            class="flex items-center gap-2 w-full text-left rounded-md -mx-1 px-1 py-1 transition-colors"
                            :class="activeTab === 'chat' ? 'bg-gray-200/70' : 'hover:bg-gray-100'"
                            @click="activeTab = 'chat'"
                        >
                            <div class="shrink-0 flex items-center justify-center w-8 h-8 rounded-lg bg-gray-200 text-base overflow-hidden">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-1.5">
                                    <span class="text-sm font-semibold text-gray-900 truncate">{{ studio.name }}</span>
                                ...
                                <p v-if="studio.description" class="text-[11px] text-gray-400 truncate">{{ studio.description }}</p>
                            </div>
                        </button>
                    </div>
                    ...
                    <nav class="px-2 py-3 flex flex-col gap-0.5">
                        <template v-for="g in navGroups" :key="g.key">
                            <div v-if="g.label && g.items.length" class="px-3 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ g.label }}</div>
                            <button
                                v-for="tab in g.items"
                                :key="tab.value"
                                type="button"
                                class="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm text-left transition-colors"
                            ...
                            >
                                ...
                                <span v-if="tab.value === 'sources' && sources.length" class="text-[11px] text-gray-400">{{ sources.length }}</span>
                            </button>
                        </template>
                    </nav>
                </aside>
                ...
                <!-- MAIN CONTENT -->
                <main class="flex-1 min-w-0 overflow-y-auto px-8 py-6 h-full">
                    <!-- Read-only banner -->
                    <div v-if="role === 'viewer'" class="mb-4 flex items-center gap-2 text-[11px] text-gray-500 bg-gray-50 border border-gray-200 rounded-md px-3 py-1.5">
                    ...
                    <div class="min-w-0">
                        ...
                        <section v-if="activeTab === 'chat'">
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <UButton v-if="canEdit" color="blue" variant="soft" size="xs" icon="i-heroicons-sparkles" :loading="improving" @click="improveNow">
                                        {{ $t('studio.improveNow') }}
                                    </UButton>
                            ...
                            <div class="mb-5 flex flex-wrap items-center gap-2">
                                <span class="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mr-1">{{ $t('studio.groundedOn') || 'Grounded on' }}</span>
                                <div
                                    v-for="s in sources"
                                    :key="s.id"
                                    class="group flex items-center gap-2 pl-2 pr-1.5 py-1.5 rounded-full border border-gray-200 bg-gray-50 hover:border-gray-300"
                                >
                                    ...
                                    <span class="text-xs text-gray-700">{{ s.name || s.agent_id }}</span>
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" :title="$t('common.connected') || 'connected'"></span>
                                    <button
                                        v-if="canEdit"
                                        class="w-4 h-4 rounded-full text-gray-300 hover:text-gray-600 hover:bg-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100"
                                ...
                                <button
                                    v-if="canEdit"
                                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-dashed border-gray-300 text-xs text-gray-500 hover:border-gray-400 hover:text-gray-700"
                                    @click="openAddSource"
                                >
                                    <UIcon name="i-heroicons-plus" class="w-3.5 h-3.5" /> {{ $t('studio.addConnection') }}
                                </button>
                            </div>
                        ...
                        <!-- QUERIES -->
                        <section v-else-if="activeTab === 'queries'">
                            <StudioQueries :studio-id="studioId" :sources="sources" :can-edit="canEdit" />
                        </section>
...
const tabs = computed(() => [
    ...
    // Data Agent parity tabs (additive). Each is scoped to the studio's pinned
    // Data Agents and reuses the existing data-agent backend routes.
    { value: 'connection', label: t('studio.tabConnection') || 'Connection', icon: 'i-heroicons-link', group: 'knowledge' },
    { value: 'tables', label: t('studio.tabTables') || 'Tables', icon: 'i-heroicons-table-cells', group: 'knowledge' },
    { value: 'tools', label: t('studio.tabTools') || 'Tools', icon: 'i-heroicons-wrench-screwdriver', group: 'knowledge' },
    { value: 'queries', label: t('studio.tabQueries') || 'Queries', icon: 'i-heroicons-code-bracket-square', group: 'knowledge' },
    ...
    { value: 'skills', label: t('studio.tabSkills'), icon: 'i-heroicons-sparkles', group: 'behavior' },
