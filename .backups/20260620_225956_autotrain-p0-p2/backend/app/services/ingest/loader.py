...
def load_dataframe_to_staging(
    ...
    *,
    ...
    """Load df into staging.<table>. Returns row count written (0 on failure).
    ...
    try:
        ...
        if load_key == "period" and period is not None:
            ...
            with engine.begin() as conn:
                exists = conn.execute(
                    ...
                    {"s": STAGING_SCHEMA, "t": tbl},
                ).first()
                if exists:
                    conn.execute(
                        text(f'DELETE FROM {STAGING_SCHEMA}."{tbl}" WHERE _period = :p'),
                        {"p": period},
                    )
            if_exists = "append"
        ...
        logger.info("loader: wrote %d rows -> %s.%s (%s)", len(stamped), STAGING_SCHEMA, tbl, load_key)
        return len(stamped)
    except Exception:
