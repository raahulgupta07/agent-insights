    ...
    *,
    ...
    if "codex" in want:
        try:
            ...
            enriched = codex.codex_enrich(
                table, schema=schema, engine=engine, llm_inference=llm_inference
            )
            if enriched:
    ...
    if "profile" in want and flags.AUTOTRAIN_PROFILE:
        try:
            ...
            prof = profiler.profile_table(table, engine=engine, schema=schema)
            if prof:
    ...
    if "qa" in want and flags.AUTOTRAIN_QA and llm_inference is not None:
        try:
            from app.services.autotrain import profiler, qa_gen

            prof = profiler.profile_table(table, engine=engine, schema=schema)
            run_sql = _make_run_sql(engine)
            pairs = qa_gen.generate_verified_qa(
                table, profile=prof, llm_inference=llm_inference, run_sql=run_sql
            )
