    ...
    <div class="flex h-full text-sm w-full">
            ...
            <template v-else-if="studio">
                ...
                <aside class="w-60 shrink-0 bg-[#FBFAF6] border-e border-[#E7E5DD] flex flex-col overflow-y-auto h-full">
                    <!-- studio header -->
                    <div class="px-3 pt-4 pb-3 border-b border-[#E7E5DD]">
                        <button class="text-xs text-[#9a958c] hover:text-[#6b6b6b] mb-2 inline-flex items-center gap-1" @click="router.push('/studios')">
                        ...
                        >
                            <div class="shrink-0 flex items-center justify-center w-8 h-8 rounded-lg bg-[#F4F1EA] border border-[#E7E5DD] text-base text-[#C2683F] overflow-hidden">
                    ...
                    <nav class="px-2 py-3 flex flex-col gap-0.5">
                        <template v-for="g in navGroups" :key="g.key">
                            <div v-if="g.label && g.items.length" class="px-3 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-[#9a958c]">{{ g.label }}</div>
                            <button
                                v-for="tab in g.items"
                                :key="tab.value"
                                type="button"
                                class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] text-left transition-colors"
                            ...
                            >
                                ...
                                <span v-if="tab.value === 'sources' && sources.length" class="text-[11px] text-[#9a958c]">{{ sources.length }}</span>
                            </button>
                        </template>
                    </nav>
                </aside>
                ...
                <main class="flex-1 min-w-0 overflow-y-auto px-8 py-6 h-full bg-[#FBFAF6]">
                    ...
                    <div class="min-w-0">
                        ...
                        <section v-else-if="activeTab === 'sources'">
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    <h2 class="text-lg font-semibold text-[#1f2328]" style="font-family: ui-serif, Georgia, 'Times New Roman', serif">{{ $t('studio.sourcesTitle') }}</h2>
                                    <p class="text-xs text-[#6b6b6b] mt-0.5">{{ $t('studio.sourcesHint') }}</p>
                                </div>
                                <button v-if="canEdit" type="button" class="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#C2683F] hover:bg-[#A8542F] rounded-lg px-3.5 py-1.5 transition-colors" @click="openAddSource">
                            ...
                            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div
                                    v-for="s in sources"
                                    :key="s.id"
                                    class="flex items-center justify-between p-3 rounded-2xl border border-[#E7E5DD] bg-white"
                                >
                                    <div class="flex items-center gap-2 min-w-0">
...
/* Claude redesign: neutralize the scope-badge palette.
   scopeBadgeClass (script computed) emits bg-blue-100/text-blue-700 (org) and
   bg-purple-100/text-purple-700 (link). The script is logic-locked, so recolor
   those exact utilities here to warm neutral / clay-soft per the mockup. */
.bg-blue-100 { background-color: #F4F1EA !important; }
.text-blue-700 { color: #6b6b6b !important; }
