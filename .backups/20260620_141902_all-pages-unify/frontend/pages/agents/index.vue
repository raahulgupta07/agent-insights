                ...
                <div v-if="allAgents.length > 0 || connections.length > 0" class="mb-6">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 my-4">
                        ...
                        <div class="flex items-center justify-end gap-3 w-full md:w-auto">
                            ...
                            <button
                                ...
                                class="w-full md:w-auto flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl bg-[#C2683F] text-white hover:bg-[#A8542F] disabled:opacity-50 whitespace-nowrap transition-colors"
                    ...
                    <div v-if="filteredAgents.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div
                            ...
                            :key="ds.id"
                            class="block p-5 rounded-2xl border border-[#E7E5DD] bg-white transition-all duration-200 group"
                        ...
                        >
                            <component :is="userHasAccess(ds) ? NuxtLink : 'div'" :to="userHasAccess(ds) ? `/agents/${ds.id}` : undefined" class="block">
                                ...
                                <!-- Metadata: data-source type -->
                                <div class="flex items-center gap-1.5 text-[11px] text-[#9a958c] mb-2">
                                    <UTooltip v-for="conn in (ds.connections || []).slice(0, 3)" :key="conn.id" :text="conn.name">
                                        <DataSourceIcon class="h-3.5" :type="conn.type" />
                                    </UTooltip>
                                    <span v-if="(ds.connections || []).length > 3" class="text-[#9a958c]">+{{ (ds.connections || []).length - 3 }}</span>
                                    <span v-if="userHasAccess(ds) && catalogFor(ds).shouldShow">{{ catalogFor(ds).label }}</span>
                                </div>

                                <!-- Connection status chip (text + colored dot, color not the only cue) -->
                                <div
                                ...
                                <p v-if="ds.description" class="mt-2 text-xs text-[#6b6b6b] leading-relaxed line-clamp-2">
                                    {{ ds.description }}
                                </p>
                                <p v-else class="mt-2 text-xs text-[#9a958c] italic">
                                    {{ $t('data.noDescription') }}
                                </p>

                                <!-- Stat row -->
                                <div class="mt-3 pt-3 border-t border-[#E7E5DD] flex items-center gap-6">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-[#1f2328] text-base leading-none"
                                            style="font-family: ui-serif, Georgia, 'Times New Roman', serif"
                                        >{{ getTableCount(ds) }}</span>
                                        <span class="text-[10px] uppercase tracking-wide text-[#9a958c] mt-1">{{ $t('data.tables') }}</span>
                                    </div>
                                    <div class="flex flex-col">
                                        <span
                                            class="text-[#1f2328] text-base leading-none"
                                            style="font-family: ui-serif, Georgia, 'Times New Roman', serif"
                                        >{{ (ds.connections || []).length }}</span>
                                        <span class="text-[10px] uppercase tracking-wide text-[#9a958c] mt-1">sources</span>
                                    </div>
                                </div>
                            </component>
                        ...
                        >
                            ...
                            <span class="mt-1 text-xs text-[#9a958c] leading-relaxed">{{ $t('data.agentsSubtitle') }}</span>
                        </button>
                    </div>
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <button
                            ...
                            class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-xl bg-[#C2683F] hover:bg-[#A8542F] text-white transition-colors"
                        >
