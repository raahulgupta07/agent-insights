            ...
            <UModal v-model="showCreate" :ui="{ width: 'sm:max-w-lg' }">
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">{{ $t('studio.persona') }}</label>
                            <UTextarea v-model="form.persona" :placeholder="$t('studio.personaPlaceholder')" :rows="2" size="sm" />
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">{{ $t('studio.avatar') }}</label>
                            <UInput v-model="form.avatar" :placeholder="$t('studio.avatarPlaceholder')" size="sm" />
                        </div>
                        <div>
...
// The list endpoint doesn't inline counts, so fetch sources/members per studio.
// Bounded by however many studios the user has; failures are swallowed per item.
const enrichCounts = async () => {
    await Promise.all(
        studios.value.map(async (s) => {
            try {
                ...
                s.source_count = (src.data.value as any[] | null)?.length ?? 0
                s.member_count = (mem.data.value as any[] | null)?.length ?? 0
            } catch {
