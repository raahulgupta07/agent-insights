<template>
  <div class="h-full w-full flex flex-col" :style="wrapperStyle">
    <div v-if="!isLoading && chartOptions && Object.keys(chartOptions).length > 0 && (data?.rows?.length || 0) > 0" class="flex-1 min-h-0">
      <VChart :key="chartKey" class="chart" :option="chartOptions" autoresize :loading="isLoading" @click="onPointClick" />
    </div>
...
watch(() => [props.step?.id, props.data?.rows, props.data_model, props.view, tokens.value, activeCrossValue.value], () => {
  buildOptions()
