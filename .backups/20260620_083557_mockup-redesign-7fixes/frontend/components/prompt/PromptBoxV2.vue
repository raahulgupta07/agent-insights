        ...
        >
            ...
            >
                <div class="flex items-center space-x-1 relative">
                    ...
                    <!-- Grounding chip (bounded-context visibility, arXiv:2605.22502).
                         Shows how many of the data source's approved tables are
                         injected per question after the top-K cap. -->
                    <UPopover v-if="contextScope.tables_total > 0" mode="hover" :popper="{ placement: 'top-start', strategy: 'fixed' }">
                        <button class="text-gray-500 hover:text-gray-900 hover:bg-gray-50 rounded-md px-2 py-1 text-xs flex items-center gap-1">
                            <Icon name="heroicons-circle-stack" class="w-3.5 h-3.5 flex-shrink-0" />
                            <span v-if="!isCompactPrompt">Grounded on {{ contextScope.tables_injected }} of {{ contextScope.tables_total }} tables</span>
                        </button>
                        <template #panel>
                            <div class="p-3 text-xs w-[240px] space-y-1.5">
                                <div class="font-medium text-gray-700">Knowledge grounding</div>
                                <div class="flex justify-between"><span class="text-gray-500">Tables injected</span><span class="text-gray-800 font-medium">{{ contextScope.tables_injected }} / {{ contextScope.tables_total }}</span></div>
                                <div v-if="contextScope.metrics_total" class="flex justify-between"><span class="text-gray-500">Metrics injected</span><span class="text-gray-800 font-medium">{{ contextScope.metrics_injected }} / {{ contextScope.metrics_total }}</span></div>
                                <div class="pt-1.5 border-t border-gray-100 text-gray-400 leading-snug">
                                    <template v-if="trimmedTables > 0">{{ trimmedTables }} table(s) trimmed per question by relevance (top-K) — keeps answers focused & cheap.</template>
                                    <template v-else>All tables fit — nothing trimmed.</template>
                                </div>
                            </div>
                        </template>
                    </UPopover>
                        ...
                        <template #panel="{ close }">
                            <div class="p-1.5 text-xs max-h-80 overflow-y-auto w-[300px]">
                                <div
                                    ...
                                    :class="selectedModel === m.id ? 'bg-blue-50/60' : 'hover:bg-gray-100'"
                                    @click="() => { selectModel(m.id); close(); }"
                                >
                                    <span v-if="selectedModel === m.id" class="absolute start-0 top-2 bottom-2 w-0.5 rounded-full bg-blue-500"></span>
                                    <div class="mt-0.5">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span
                                                ...
                                                :class="modelMeta(m).tier === 'pro' ? 'bg-indigo-100 text-indigo-600' : 'bg-emerald-100 text-emerald-600'"
                                            >{{ modelMeta(m).tierLabel }}</span>
                                            <Icon v-if="selectedModel === m.id" name="heroicons-check" class="w-3.5 h-3.5 text-blue-500 ms-auto shrink-0" />
                                        </div>
                                        ...
                                        <div class="flex flex-wrap gap-1 mt-1.5">
                                            <span
                                                ...
                                                :key="i"
                                                class="text-[10px] text-gray-500 bg-gray-100 rounded px-1.5 py-0.5"
                                            >{{ c }}</span>
                                        </div>
                                    </div>
                                </div>
