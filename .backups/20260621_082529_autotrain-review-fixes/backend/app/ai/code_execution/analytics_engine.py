...
def _is_writable_schema(schema: str) -> bool:
    """Writable targets for the AGENT-facing guarded engine: analytics + the shared
    staging only. Per-org `staging_<orgid>` writes do NOT go through this engine —
    the autotrain loader uses a dedicated raw server engine with a server-derived
    schema, so the agent (Engineer) can never write into another org's staging_<x>."""
    return schema == ANALYTICS_SCHEMA or schema == STAGING_SCHEMA
