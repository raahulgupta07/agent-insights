...
async def _ingest_one(
    db, *, organization, data_source, batch, table_name, df, content_hash, source_file, load_key, period, model
) -> dict:
    """Contract-check -> load to staging -> autotrain one table. Returns a result dict."""
    from app.services.autotrain import orchestrator
    from app.services.ingest import gate, loader
    ...
    summary = await orchestrator.autotrain(
        ...
        table=safe, schema="staging", model=model,
    )
    return {"table": safe, "ok": True, "rows": rows, "gate": g, "contract": contract, "autotrain": summary}
...
async def autotrain_from_file(
    ...
    for tname, df in tables:
        r = await _ingest_one(
            db, organization=organization, data_source=ds, batch=batch,
