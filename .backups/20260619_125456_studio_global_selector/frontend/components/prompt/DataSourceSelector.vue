        ...
        <UPopover :popper="{ strategy: 'absolute', placement: 'bottom-start', offset: [0,8] }">
            ...
            <template #panel>
                <div class="p-2 text-xs max-h-64 overflow-y-auto w-max min-w-[260px] max-w-[420px] rounded-xl">
                    ...
                    <template v-else>
                        <div v-if="visibleDataSources.length === 0 && connectableDataSources.length === 0 && studios.length === 0" class="text-center text-gray-500 py-4">
                        ...
                        <template v-else>
                            ...
                            <template v-if="studios.length > 0">
                                <div class="px-2 pt-1 pb-1 text-[10px] uppercase tracking-wide text-gray-400 font-semibold">Studios</div>
                                <div
                                    v-for="s in studios"
                                    :key="s.id"
                                ...
                                >
                                    ...
                                    <Icon v-if="internalSelectedStudioId === s.id" name="heroicons-check" class="w-4 h-4 text-blue-500 flex-shrink-0" />
                                </div>
...
// `GET /studios` returns [] when HYBRID_STUDIOS is off, so this self-gates.
...
const selectedStudio = computed<Studio | null>(() =>
    studios.value.find(s => s.id === internalSelectedStudioId.value) || null
)
...
async function getStudios() {
    // Self-gating: backend returns [] when HYBRID_STUDIOS is off, so no section renders.
    try {
        const response = await useMyFetch('/studios', { method: 'GET' })
        studios.value = ((response.data.value as any[]) || []).map((s: any) => ({
