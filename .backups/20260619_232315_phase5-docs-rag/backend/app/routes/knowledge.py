...
additive on top of bow's schema store (DataSourceTable -> ConnectionTable, which
...
def _enumerate_schema(data_source: DataSource) -> list[tuple[str, list[dict]]]:
    """Return [(table_name, [{name, type}, ...]), ...] from bow's schema store.
...
async def test_metric(
    ...
    Reuses bow's data-source query path: DataSource.get_client().aexecute_query()
