...
<script setup lang="ts">
  ...
  const mainNavItems: NavItem[] = [
    { href: '/studios', icon: 'heroicons-film', label: 'nav.studios' },
  ...
  const { initAgent, selectedAgentObjects, agents, hasAgents, selectedStudioId } = useAgent()
...
const createNewReport = async () => {
  ...
  try {
    ...
    // the chat inherits its pinned sources + persona/instructions. When no studio
    // is active the body stays identical to the original shape (no null key).
