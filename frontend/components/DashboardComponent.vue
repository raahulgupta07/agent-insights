<template>
    <div class="flex flex-col h-full overflow-hidden">
        ...
        <div class="relative flex-1 overflow-y-auto dashboard-area bg-white" :style="wrapperStyle">
            ...
            <!-- P4: saved views (bookmarks) + export -->
            <div v-if="!isLoading" class="flex items-center gap-2 px-3 pt-1.5 flex-wrap">
                <div class="flex items-center gap-1 flex-wrap">
                    ...
                    <button
                        ...
                        @click="saveBookmark"
                    >
                        <Icon name="heroicons:bookmark" class="w-3 h-3" /> Save view
                    </button>
                </div>
                <div class="ms-auto">
                ...
                <div v-for="widget in allWidgets"
                    ...
                    >
                        <WidgetControls
                            ...
                            @format="openFormatEditor(widget)"
                            @fields="openFieldEditor(widget)"
                        />
        ...
        <!-- Conditional formatting editor (P3) -->
        <FormatRulesEditor
            :open="formatEditorOpen"
            :widget="formatEditorWidget"
            @close="formatEditorOpen = false"
            @apply="applyFormat"
        />

        <!-- Field-well chart builder (P2) -->
        <FieldWellEditor
...
import FormatRulesEditor from '@/components/dashboard/FormatRulesEditor.vue';
import FieldWellEditor from '@/components/dashboard/FieldWellEditor.vue';
...
import { themes } from '@/components/dashboard/themes'
    ...
    // --- Conditional formatting editor (P3) ---
    const formatEditorOpen = ref(false);
    const formatEditorWidget = ref<any | null>(null);

    function openFormatEditor(widget: any) {
        formatEditorWidget.value = widget;
        formatEditorOpen.value = true;
    }

    // Apply formatting rules to a widget's view, persist into layout_view_overrides,
    // and live-update the displayedWidgets source so the renderer re-renders at once.
    function applyFormat(payload: { conditionalRules?: any[]; dataBars?: any; kpiTarget?: any }) {
        const id = formatEditorWidget.value?.id;
        formatEditorOpen.value = false;
        if (!id) return;
        const idx = displayedWidgets.value.findIndex((w: any) => w.id === id);
        if (idx < 0) return;
        const w = displayedWidgets.value[idx];

        // Only the keys provided by the editor are written (others preserved).
        const ruleKeys: Record<string, any> = {};
        if ('conditionalRules' in payload) ruleKeys.conditionalRules = payload.conditionalRules;
        if ('dataBars' in payload) ruleKeys.dataBars = payload.dataBars;
        if ('kpiTarget' in payload) ruleKeys.kpiTarget = payload.kpiTarget;

        const lvo = { ...(w.layout_view_overrides || {}) };
        lvo.view = { ...(lvo.view || {}), ...ruleKeys };
        const view = { ...(w.view || {}) };
        view.view = { ...(view.view || {}), ...ruleKeys };

        const next = [...displayedWidgets.value];
        next[idx] = { ...w, layout_view_overrides: lvo, view };
        displayedWidgets.value = next;

        schedulePersistFullLayout();
    }

    // ---- P2: field-well chart builder ----
    const fieldEditorOpen = ref(false);
    const fieldEditorWidget = ref<any | null>(null);

    function openFieldEditor(widget: any) {
        fieldEditorWidget.value = widget;
        fieldEditorOpen.value = true;
    }

    // Apply field selections (type/x/y/groupBy/aggregation) to a widget's view.
    // A key with `undefined` value is REMOVED (reverts that field to the agent's auto choice).
    function applyFields(payload: { type?: string; x?: string; y?: string; groupBy?: string; aggregation?: string }) {
        const id = fieldEditorWidget.value?.id;
        ...
        const FIELD_KEYS: Array<keyof typeof payload> = ['type', 'x', 'y', 'groupBy', 'aggregation'];
        for (const k of FIELD_KEYS) {
            if (!(k in payload)) continue;
            const val = payload[k];
            if (val === undefined || val === '') {
                delete (lvo.view as any)[k];
                delete (view.view as any)[k];
            } else {
                (lvo.view as any)[k] = val;
                (view.view as any)[k] = val;
            }
        }
    ...
    // ---- P4: bookmarks / saved views (localStorage, per report) ----
    const bookmarks = ref<Array<{ id: string; name: string; filters: any[]; theme: string }>>([]);
    const activeBookmarkId = ref('');
    const bookmarkKey = computed(() => `dash_bookmarks_${props.report?.id || 'unknown'}`);

    function loadBookmarks() {
        try {
            const raw = typeof localStorage !== 'undefined' ? localStorage.getItem(bookmarkKey.value) : null;
            bookmarks.value = raw ? JSON.parse(raw) : [];
        } catch { bookmarks.value = []; }
    }
    function persistBookmarks() {
        try { if (typeof localStorage !== 'undefined') localStorage.setItem(bookmarkKey.value, JSON.stringify(bookmarks.value)); } catch { /* ignore */ }
    }
    function saveBookmark() {
        const name = (window.prompt('Name this view', `View ${bookmarks.value.length + 1}`) || '').trim();
        if (!name) return;
        bookmarks.value = [...bookmarks.value, {
            id: `bm_${Date.now()}`,
            name,
            filters: JSON.parse(JSON.stringify(crossFilters.value || [])),
            theme: themeOverride.value || '',
        }];
        persistBookmarks();
    }
    function applyBookmark(bm: any) {
        crossFilters.value = JSON.parse(JSON.stringify(bm.filters || []));
        crossVersion.value++;
        if (bm.theme) themeOverride.value = bm.theme;
    ...
    watch(() => props.report?.id, () => { loadBookmarks(); activeBookmarkId.value = ''; }, { immediate: true });

    // ---- P4: export (data xlsx / image png / pdf / pptx) ----
    const isExporting = ref(false);

    function exportBaseName() {
        return String(props.report?.title || 'dashboard').replace(/[^\w\-]+/g, '_').slice(0, 40) || 'dashboard';
    }
    function exportableWidgets() {
        return (displayedWidgets.value || []).filter((w: any) =>
            w?.isVisualization && Array.isArray(w?.last_step?.data?.rows) && w.last_step.data.rows.length);
    }
    function widgetSheetName(w: any, i: number) {
        const base = String(w?.title || w?.view?.title || w?.last_step?.title || `Widget ${i + 1}`)
            .replace(/[\\/?*[\]:]/g, ' ').trim().slice(0, 28);
        return base || `Sheet ${i + 1}`;
    }
    ...
    async function exportData() {
        try {
            ...
            exportableWidgets().forEach((w: any, i: number) => {
                let name = widgetSheetName(w, i);
                while (used.has(name)) name = (name.slice(0, 25) + ' ' + i).slice(0, 28);
                used.add(name);
                XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(w.last_step.data.rows), name);
            });
            if (!wb.SheetNames.length) XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['No data']]), 'Empty');
            const out = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
            downloadBlob(new Blob([out], { type: 'application/octet-stream' }), `${exportBaseName()}.xlsx`);
        } catch (e) { console.error('export data failed', e); }
    }
    async function captureCanvas() {
        const el = gridstackContainer.value;
        if (!el) return null;
        const h2c: any = (await import('html2canvas')).default;
        return await h2c(el, { backgroundColor: '#ffffff', scale: 2, useCORS: true, logging: false });
    }
    async function exportImage() {
        try {
            ...
            if (!canvas) return;
            canvas.toBlob((b: Blob | null) => { if (b) downloadBlob(b, `${exportBaseName()}.png`); }, 'image/png');
        } catch (e) { console.error('export png failed', e); } finally { isExporting.value = false; }
    }
    async function exportPdf() {
        try {
            ...
            pdf.addImage(img, 'PNG', (pw - w) / 2, 12, w, h);
            pdf.save(`${exportBaseName()}.pdf`);
        } catch (e) { console.error('export pdf failed', e); } finally { isExporting.value = false; }
    }
    async function exportPptx() {
        try {
            ...
            const canvas = await captureCanvas();
            if (!canvas) return;
            const Pptx: any = (await import('pptxgenjs')).default;
            const pptx = new Pptx();
            pptx.defineLayout({ name: 'W', width: 13.33, height: 7.5 });
            pptx.layout = 'W';
            const slide = pptx.addSlide();
            slide.addText(String(props.report?.title || 'Dashboard'), { x: 0.4, y: 0.2, fontSize: 18, bold: true, color: '1f2328' });
            const img = canvas.toDataURL('image/png');
            const ratio = Math.min(12.5 / canvas.width, 6.4 / canvas.height);
            const w = canvas.width * ratio, h = canvas.height * ratio;
            slide.addImage({ data: img, x: (13.33 - w) / 2, y: 0.9, w, h });
            await pptx.writeFile({ fileName: `${exportBaseName()}.pptx` });
        } catch (e) { console.error('export pptx failed', e); } finally { isExporting.value = false; }
    }
    const exportMenuItems = computed(() => [[
        { label: 'Data (.xlsx)', icon: 'i-heroicons-table-cells', click: exportData },
        { label: 'Image (.png)', icon: 'i-heroicons-photo', click: exportImage },
        { label: 'PDF (.pdf)', icon: 'i-heroicons-document-text', click: exportPdf },
        { label: 'Slides (.pptx)', icon: 'i-heroicons-presentation-chart-bar', click: exportPptx },
    ]]);
