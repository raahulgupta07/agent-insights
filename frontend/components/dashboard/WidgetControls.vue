<template>
  <div v-if="edit" class="absolute end-1 top-1 z-20 flex gap-1 p-1 rounded ">
    ...
    <button v-if="!isText && (widget?.type === 'regular' || widget?.type === 'visualization' || isVisualization)" title="Build chart fields" class="text-xs items-center flex gap-0.5 hover:bg-[#F4E5DA] text-[#C2541E] px-1 py-0.5 rounded " @click="$emit('fields')">
      <Icon name="heroicons:adjustments-horizontal" class="w-3 h-3"/>
    </button>
    <button v-if="!isText && (widget?.type === 'regular' || widget?.type === 'visualization' || isVisualization)" title="Conditional formatting" class="text-xs items-center flex gap-0.5 hover:bg-[#F4E5DA] text-[#C2541E] px-1 py-0.5 rounded " @click="$emit('format')">
      <Icon name="heroicons:swatch" class="w-3 h-3"/>
    </button>
    <button v-if="isVisualization && queryId" title="Edit Query" class="text-xs items-center flex gap-0.5 hover:bg-[#F4E5DA] text-[#C2541E] px-1 py-0.5 rounded " @click="handleEditVisualization">
      <Icon name="heroicons:pencil-square" class="w-3 h-3"/>
    </button>
    <button v-if="isText" :title="isNew ? 'Cancel Adding Text' : 'Edit Text'" class="text-xs items-center flex gap-0.5 hover:bg-[#F4E5DA] text-[#C2541E] px-1 py-0.5 rounded " @click="$emit('toggleTextEdit')">
