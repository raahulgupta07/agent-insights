<template>
  <div class="h-full w-full flex flex-col" :style="wrapperStyle">
    ...
    <div v-if="!isLoading && chartOptions && Object.keys(chartOptions).length > 0 && (data?.rows?.length || 0) > 0" class="flex-1 min-h-0" @contextmenu.prevent>
      <VChart :key="chartKey" class="chart" :option="chartOptions" autoresize :loading="isLoading" @click="onPointClick" @contextmenu="onContextMenu" />
    </div>
...
import { toRefs, ref, watch, computed, inject } from 'vue'
...
import { TitleComponent, TooltipComponent, GridComponent, LegendComponent, VisualMapComponent, DataZoomComponent, MarkLineComponent, MarkPointComponent, AriaComponent, ToolboxComponent } from 'echarts/components'

use([
  CanvasRenderer,
  PieChart, BarChart, LineChart, ScatterChart, HeatmapChart, CandlestickChart, TreemapChart, RadarChart,
  TitleComponent, TooltipComponent, GridComponent, LegendComponent, VisualMapComponent, DataZoomComponent, MarkLineComponent, MarkPointComponent, AriaComponent, ToolboxComponent,
])
...
// Cross-filter API provided by DashboardComponent via provide('dashCrossFilter', ...).
// NULL when this chart renders outside a dashboard (MetricCard, previews) → everything no-ops.
const crossFilter = inject<any>('dashCrossFilter', null)

// The column this chart cross-filters on. Mirrors the key logic in buildCartesianOptions
// (categoryKey = viewV2?.x || dm?.series?.[0]?.key) and buildPieOptions (cfg.key), lowercased.
const clickDimension = computed<string | null>(() => {
  const dm = props.data_model || props.data?.data_model || null
  const viewV2 = props.view?.view || null
  const k = viewV2?.x || dm?.series?.[0]?.key || (dm as any)?.x || null
  return k ? String(k).toLowerCase() : null
})

// Currently-active cross-filter value for THIS chart's dimension (for highlight/dim).
const activeCrossValue = computed<string | null>(() => {
  if (!crossFilter || !clickDimension.value) return null
  const f = (crossFilter.state?.value || []).find((x: any) => x.column === clickDimension.value)
  return f ? String(f.value) : null
})

function onPointClick(params: any) {
  if (!crossFilter || !clickDimension.value) return
  const val = params?.name
  if (val == null || val === '') return
  crossFilter.apply(clickDimension.value, String(val), clickDimension.value)
}

// Per-datum opacity for cross-filter highlight: matching/none-active → 1, non-matching → 0.35.
function crossOpacity(cat: any): number {
  const active = activeCrossValue.value
  if (!active) return 1
  return String(cat).toLowerCase() !== active.toLowerCase() ? 0.35 : 1
}

// P3 conditional formatting: per-datum color rules read from the widget view JSON.
const condRules = computed<any[]>(() =>
  Array.isArray(props.view?.view?.conditionalRules) ? props.view!.view!.conditionalRules : []
)

// Evaluate a single rule against a numeric value. Never throws. First matching rule wins.
function matchRule(v: number, r: { op: string; value: number; value2?: number }): boolean {
  ...
  switch (r.op) {
    ...
    case '==': return v === r.value
    case '!=': return v !== r.value
    case 'between': return r.value2 != null && v >= Math.min(r.value, r.value2) && v <= Math.max(r.value, r.value2)
    default: return false
  }
}

// Return the rule color for the first matching rule, or null when none match / no rules.
function condColorFor(v: any): string | null {
  const rules = condRules.value
  if (!rules.length || v == null || Number.isNaN(Number(v))) return null
  const num = Number(v)
  for (const r of rules) {
    try {
      if (r && matchRule(num, r) && typeof r.color === 'string' && r.color) return r.color
    } catch { /* never throw */ }
  }
  return null
}

// ---- P2b drill-down: per-widget hierarchical drill (LOCAL, not persisted) ----
// Right-click a category → step into the next dimension, filtered to the clicked value.
const drillStack = ref<{ column: string; value: string }[]>([])

// Candidate dimension columns from the rows (non-numeric values), in encounter order, lowercased.
function dimensionColumns(): string[] {
...
// Effective category key given the chart's base key and current drill depth.
function effectiveCatKey(baseKey: string | null | undefined): string | null {
  const bk = baseKey ? String(baseKey).toLowerCase() : null
  if (!drillStack.value.length) return bk
  const used = new Set<string>([...(bk ? [bk] : []), ...drillStack.value.map(d => d.column)])
  const next = dimensionColumns().find(c => !used.has(c))
  return next || bk
}

// Rows filtered by the active drill path (case-insensitive equality on each step).
function drilledRows(rows: any[]): any[] {
  if (!drillStack.value.length || !Array.isArray(rows)) return rows
  return rows.filter(r => drillStack.value.every(d => {
    const k = (d.column in r) ? d.column : Object.keys(r).find(kk => kk.toLowerCase() === d.column)
    if (!k) return true // row lacks the column → unaffected
    return String(r[k]).toLowerCase() === d.value.toLowerCase()
  }))
}
...
function onContextMenu(params: any) {
  const val = params?.name
  if (val == null || val === '') return
  const curKey = effectiveCatKey(clickDimension.value)
  if (!curKey) return
  const used = new Set<string>([curKey, ...drillStack.value.map(d => d.column)])
  const hasDeeper = dimensionColumns().some(c => !used.has(c))
  if (!hasDeeper) return // no further dimension to drill into
  drillStack.value = [...drillStack.value, { column: curKey, value: String(val) }]
}

// Keep `level` steps (0 = back to top).
...
function buildPieOptions(rows: any[], dm: any): EChartsOption {
  ...
  const keyLower = effectiveCatKey(cfg.key) || String(cfg.key).toLowerCase()
  ...
  if (aggFn) {
    ...
    data = Array.from(grouped.entries())
      .map(([name, vals], i: number) => {
        const value = aggregateValues(vals, aggFn)
        const ruleColor = condColorFor(value)
        return {
          name,
          value,
          itemStyle: { color: ruleColor || resolveColorInput(colors[i % colors.length]), opacity: crossOpacity(name) }
        }
      })
    ...
    data = rows
      .map((r: any, i: number) => {
        const value = Number(r[valLower])
        const ruleColor = condColorFor(value)
        return {
          name: r[keyLower],
          value,
          itemStyle: { color: ruleColor || resolveColorInput(colors[i % colors.length]), opacity: crossOpacity(r[keyLower]) }
        }
      })
  ...
  return {
    ...
    toolbox: { right: 8, top: 2, itemSize: 13, feature: { saveAsImage: { title: 'Save' } }, iconStyle: { borderColor: '#9a958c' } },
...
function buildCartesianOptions(rows: any[], dm: any): EChartsOption {
  ...
  const viewV2 = props.view?.view
  const categoryKey = effectiveCatKey(viewV2?.x || dm?.series?.[0]?.key)
  ...
  if (groupByKey) {
    ...
    const crossActive = activeCrossValue.value !== null
    series = groups.map((group: string, i: number) => {
      ...
      const data = categories.map(cat => {
        ...
        const agg = aggregateValues(values, aggFn)
        const ruleColor = agg != null ? condColorFor(agg) : null
        if (ruleColor) return { value: agg, itemStyle: { color: ruleColor, opacity: crossActive ? crossOpacity(cat) : 1 } }
        if (crossActive && agg != null) return { value: agg, itemStyle: { opacity: crossOpacity(cat) } }
        return agg
      })
  ...
  return {
    ...
    toolbox: { right: 8, top: 2, itemSize: 13, feature: { dataZoom: { yAxisIndex: 'none' }, saveAsImage: { title: 'Save' }, restore: { title: 'Reset' } }, iconStyle: { borderColor: '#9a958c' } },
...
function buildOptions() {
  ...
  const rows = drilledRows(normalizeRows(props.data?.rows))
...
watch(() => [props.step?.id, props.data?.rows, props.data_model, props.view, tokens.value, activeCrossValue.value, drillStack.value], () => {
  buildOptions()
