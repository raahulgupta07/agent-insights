    ...
    <NuxtLayout name="default">
        <div class="flex justify-center ps-2 md:ps-4 text-sm">
            <div class="w-full max-w-7xl px-4 ps-0 py-4">
                ...
                <!-- Back link -->
                <NuxtLink to="/agents" class="inline-flex items-center gap-1 text-xs text-[#9a958c] hover:text-[#6b6b6b] transition-colors">
                        ...
                        <template v-else-if="!fetchError">
                            ...
                            <div class="flex items-center gap-3 mt-4 flex-wrap">
                                <template v-if="(integration?.connections || []).length > 0">
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium text-[#3f9e6a] bg-[#eef6f0]">
                                        <span class="inline-block w-1.5 h-1.5 rounded-full bg-[#3f9e6a]" />
                                        Connected
                                    </span>
                                    <span class="text-[#9a958c] select-none">·</span>
                                    <div
                                    ...
                                    >
                                        ...
                                        <UTooltip :text="conn.name + (getEffectiveStatus(conn) === 'indexing' ? ' · ' + indexingSummary(conn.indexing) : '')">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <span class="text-xs text-[#6b6b6b]">{{ conn.name }}</span>
                                            </div>
                                        </UTooltip>
                                    </div>
                                    <span v-if="(integration.connections || []).length > 4" class="text-xs text-[#9a958c]">
                                        +{{ integration.connections.length - 4 }}
                                    </span>
                                </template>
                                <span v-else class="text-xs text-[#9a958c] italic">No connections</span>

                                <template v-if="(catalog.shouldShow && catalog.count > 0) || connectionCount > 0">
                                    <template v-if="catalog.shouldShow">
                                        <span class="text-[#9a958c] select-none">·</span>
                                        <span class="text-xs text-[#9a958c]">{{ catalog.label }}</span>
                                    </template>
                                    <span class="text-[#9a958c] select-none">·</span>
                                    <span class="text-xs text-[#9a958c]">{{ connectionCount }} {{ connectionCount === 1 ? 'connection' : 'connections' }}</span>
                                </template>
                            </div>
                        </template>
