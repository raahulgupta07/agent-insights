<template>
    <div class="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div v-else class="space-y-5">
            <div v-for="grp in groups" :key="grp.scope_kind + grp.scope_key"
                ...
                <div class="px-4 py-3 border-b border-[#F1EFEC] flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <span class="text-[10.5px] px-2 py-0.5 rounded-full bg-[#EEF2FF] text-[#4F46E5] border border-[#C7D2FE]">{{ grp.scope_kind }}</span>
                        <h3 class="text-[13px] font-mono text-[#57534E] truncate max-w-[360px]">{{ grp.scope_key }}</h3>
                    </div>
