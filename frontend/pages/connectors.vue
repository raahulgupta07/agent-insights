        ...
        <div class="w-full max-w-7xl py-2 text-[#1f2328]">
            ...
            <div class="flex items-start justify-between gap-4 mb-6">
                ...
                <button
                    ...
                    class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl bg-[#C2683F] text-white hover:bg-[#A8542F] transition-colors whitespace-nowrap shrink-0"
                >
            ...
            <div v-if="!canCreateDataSource" class="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 class="text-[15px] font-semibold text-[#1f2328]" style="font-family: ui-serif, Georgia, 'Times New Roman', serif">Admins only</h3>
            ...
            <template v-else>
                ...
                <div v-if="loading" class="flex items-center justify-center py-20">
                    <Spinner class="w-6 h-6 text-[#C2683F] animate-spin" />
                </div>
                ...
                <template v-else>
                    ...
                    >
                        <span class="inline-flex w-11 h-11 items-center justify-center rounded-xl bg-white border border-[#E7E5DD] text-[#C2683F] shrink-0">
                            <UIcon name="i-heroicons-cloud-arrow-up" class="w-6 h-6" />
                        </span>
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <span class="text-[15px] font-semibold text-[#1f2328]" style="font-family: ui-serif, Georgia, 'Times New Roman', serif">
                                    Upload File / Spreadsheet
                                </span>
                                <span class="inline-flex items-center rounded-full bg-[#C2683F] px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white">new</span>
                            </div>
                            <p class="mt-0.5 text-xs text-[#6b6b6b]">.xlsx · .xls · .csv → instant Data Agent</p>
                        </div>
                        <UIcon name="i-heroicons-arrow-right" class="ms-auto w-5 h-5 text-[#A8542F] opacity-0 transition group-hover:opacity-100 shrink-0" />
                    </button>
                    ...
                    <div v-if="connections.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <button
                            ...
                            class="flex flex-col gap-3 rounded-2xl border border-[#E7E5DD] bg-white p-4 text-start cursor-pointer transition hover:-translate-y-0.5 hover:shadow-md"
                        >
                            <div class="flex items-center justify-between gap-2 rounded-xl border border-[#E7E5DD] bg-[#F4F1EA] px-3 py-2">
                                <span class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-white border border-[#E7E5DD]">
                            ...
                            <div class="text-[15px] font-semibold text-[#1f2328] truncate" style="font-family: ui-serif, Georgia, 'Times New Roman', serif">
                                {{ conn.name }}
                            </div>
                            <p class="text-xs text-[#6b6b6b] leading-relaxed capitalize">{{ conn.type }}</p>
                            <div class="border-t border-[#E7E5DD] pt-2 mt-1 text-[11px] text-[#9a958c] flex items-center gap-1.5">
