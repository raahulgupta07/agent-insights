<template>
  <div class="h-full overflow-y-auto bg-[#F5F4EE] flex items-start justify-center px-4 py-8">
    <div class="w-full max-w-2xl bg-white border border-[#E9E0D3] rounded-2xl shadow-sm overflow-hidden">
      <!-- ── Step crumb ─────────────────────────────────────────────── -->
      <div class="px-6 pt-5">
          ...
          <template v-for="(s, i) in stepLabels" :key="s">
            <div class="flex items-center gap-2 text-[11px]"
              ...
              <span class="w-[22px] h-[22px] rounded-full border flex items-center justify-center text-[11px] font-bold"
                :class="step === i + 1
                  ? 'bg-[#C2541E] border-[#C2541E] text-white'
                  : step > i + 1
                    ? 'bg-[#E7F2EC] border-[#cde6d8] text-[#3f9e6a]'
                    : 'bg-white border-[#E9E0D3] text-[#9a958c]'">
            ...
            <div v-if="i < stepLabels.length - 1" class="flex-1 h-[1.5px] mx-0.5"
              :class="step > i + 1 ? 'bg-[#cde6d8]' : 'bg-[#E9E0D3]'"></div>
          </template>
        </div>
      </div>
      ...
      <div class="px-6 pt-2 min-h-[320px]">
        <!-- STEP 1 — NAME -->
        <div v-if="step === 1">
          <h2 class="text-[19px] font-semibold" style="font-family: 'Spectral', ui-serif, Georgia, serif">Name your agent</h2>
          ...
          <input
            ...
            class="w-full border border-[#E9E0D3] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#C2541E] focus:ring-2 focus:ring-[#FBF6F2]"
          />
          ...
          <div class="inline-flex border border-[#E9E0D3] rounded-lg overflow-hidden">
            <button
              v-for="opt in shareOptions"
              :key="opt.value"
              type="button"
              class="text-xs px-3 py-1.5 border-l first:border-l-0 border-[#E9E0D3]"
              :class="shareScope === opt.value ? 'bg-[#FBF6F2] text-[#A8330F] font-semibold' : 'bg-white text-[#6b6b6b]'"
        ...
        <div v-else-if="step === 2">
          <h2 class="text-[19px] font-semibold" style="font-family: 'Spectral', ui-serif, Georgia, serif">Add your data</h2>
          <p class="text-[12.5px] text-[#6b6b6b] mt-0.5 mb-4">
            Upload a file or pin a data agent your organization already has.
          </p>

          <div class="inline-flex border border-[#E9E0D3] rounded-lg overflow-hidden mb-3">
            <button
              type="button"
              class="text-xs px-3 py-1.5"
              :class="dataMode === 'upload' ? 'bg-[#FBF6F2] text-[#A8330F] font-semibold' : 'bg-white text-[#6b6b6b]'"
              @click="dataMode = 'upload'"
            >Upload file</button>
            <button
              type="button"
              class="text-xs px-3 py-1.5 border-l border-[#E9E0D3]"
              :class="dataMode === 'pin' ? 'bg-[#FBF6F2] text-[#A8330F] font-semibold' : 'bg-white text-[#6b6b6b]'"
              @click="onSelectPin"
            >Pin existing</button>
          </div>
          ...
          <div v-if="dataMode === 'upload'">
            <div
              class="border-2 border-dashed border-[#E8C9B5] rounded-xl p-6 text-center cursor-pointer transition hover:border-[#C2541E] hover:bg-[#FBF6F2]"
              @click="fileInput?.click()"
              @dragover.prevent
              @drop.prevent="onDrop"
            >
              <UIcon name="i-heroicons-arrow-up-tray" class="w-8 h-8 mx-auto text-[#C2541E]" />
              <div class="text-[13px] font-semibold text-[#C2541E] mt-2">Click or drag .csv / .xlsx</div>
            ...
            <div v-if="uploadFileName" class="mt-3 flex items-center gap-2.5 border border-[#F0EEE6] bg-[#F6F1EA] rounded-lg px-3 py-2 text-[12.5px]">
              <span class="w-6 h-6 rounded-md bg-white border border-[#F0EEE6] flex items-center justify-center">
                <UIcon v-if="uploading" name="i-heroicons-arrow-path" class="w-3.5 h-3.5 text-[#C2541E] animate-spin" />
            ...
            <div v-else class="space-y-2 max-h-[220px] overflow-y-auto">
              <button
                ...
                :class="pinnedAgentId === String(a.id)
                  ? 'border-[#C2541E] bg-[#FBF6F2]'
                  : 'border-[#E9E0D3] hover:border-[#d8d4ca]'"
                @click="pinnedAgentId = String(a.id)"
              >
                <UIcon name="i-heroicons-circle-stack" class="w-4 h-4 text-[#C2541E] shrink-0" />
                <span class="truncate flex-1">{{ a.name || 'Data source' }}</span>
                <UIcon v-if="pinnedAgentId === String(a.id)" name="i-heroicons-check-circle" class="w-4 h-4 text-[#C2541E]" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="step === 3" class="text-center">
          <h2 class="text-[19px] font-semibold" style="font-family: 'Spectral', ui-serif, Georgia, serif">Training your agent</h2>
          <p class="text-[12.5px] text-[#6b6b6b] mt-0.5 mb-4">
          ...
          <div class="relative w-24 h-24 mx-auto mb-3">
            <svg width="96" height="96" style="transform: rotate(-90deg)">
              ...
              <circle
                cx="48" cy="48" r="40" stroke="#C2541E" stroke-width="9" fill="none" stroke-linecap="round"
                stroke-dasharray="251" :stroke-dashoffset="251 - 251 * trainPct / 100"
                style="transition: stroke-dashoffset .4s ease"
              />
            </svg>
            <div class="absolute inset-0 flex flex-col items-center justify-center">
              <span class="text-2xl" style="font-family: 'Spectral', ui-serif, Georgia, serif">{{ trainPct }}</span>
        ...
        <div v-else-if="step === 4" class="text-center">
          ...
          <h2 class="text-[19px] font-semibold" style="font-family: 'Spectral', ui-serif, Georgia, serif">
          ...
          <div v-if="readyCounts.length" class="grid grid-cols-3 gap-2 mt-2">
            <div v-for="c in readyCounts" :key="c.label" class="border border-[#E9E0D3] rounded-lg py-2 text-center">
              <div class="text-[17px] font-bold" style="font-family: 'Spectral', ui-serif, Georgia, serif">{{ c.n }}</div>
          ...
          <div class="flex flex-col gap-2 mt-5">
            <button
              type="button"
              class="text-[13px] font-semibold rounded-lg px-4 py-2.5 bg-[#C2541E] hover:bg-[#A8330F] text-white"
              @click="openAgent"
            >Open agent</button>
            <button
              type="button"
              class="text-[13px] font-semibold rounded-lg px-4 py-2.5 border border-[#E9E0D3] text-[#1f2328] hover:bg-[#F6F1EA]"
              @click="startChatting"
      ...
      <div class="flex justify-between items-center px-6 py-4 border-t border-[#F0EEE6] mt-4">
        ...
        <div class="flex gap-2">
          ...
          <button
            v-if="step === 3"
            type="button"
            class="text-[13px] font-semibold rounded-lg px-4 py-2 border border-[#E9E0D3] text-[#6b6b6b] hover:bg-[#F6F1EA]"
            @click="skipTraining"
          ...
          <button
            v-if="step < 4"
            type="button"
            class="text-[13px] font-semibold rounded-lg px-[18px] py-2 bg-[#C2541E] hover:bg-[#A8330F] text-white disabled:opacity-50 disabled:cursor-default inline-flex items-center gap-1.5"
