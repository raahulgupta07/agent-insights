...
def _build_prompt(table: str, profile: dict, max_pairs: int, schema: str = "staging") -> str:
    ...
    return (
        f"You are a data analyst. The table is {ref}.\n"
        ...
        f"Each SQL must be a single read-only SELECT over {ref} "
...
def generate_verified_qa(table: str, *, profile: dict, llm_inference, run_sql, max_pairs: int = 8, schema: str = "staging") -> list[dict]:
    try:
        prompt = _build_prompt(table, profile or {}, max_pairs, schema=schema)
