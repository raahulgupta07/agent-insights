...
def profile_table(table: str, *, engine=None, schema: str = "staging", max_distinct: int = 500) -> dict:
    ...
    try:
        if engine is None:
            # write engine is read-capable AND pgbouncer-safe (the readonly engine
            # sets default_transaction_read_only in startup options, which pgbouncer
            # txn-pooling rejects). SELECTs still pass the write-guard.
