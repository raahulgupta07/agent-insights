...
class CompletionFeedbackService:
    ...
    async def create_or_update_feedback(
        ...
        if existing_feedback:
            ...
            # Domain Packs Phase 5: thumbs -> pack win-rate (demotes losers). Gated.
            self._maybe_record_pack_signal(
                completion_id=completion_id,
                organization=organization,
                direction=existing_feedback.direction,
            )
    ...
    def _maybe_record_pack_signal(
        ...
        *,
        ...
        """Domain Packs Phase 5: turn a thumbs vote into a pass/fail on the pack
        that fired for this completion (PackWinrate). Fires for BOTH directions
        (win-rate needs wins AND losses). Cheap predicate up front; the lookup +
        upsert runs fire-and-forget in its own DB session. No-op unless
        DOMAIN_PACKS. Failures are swallowed — never surface to the feedback POST."""
        try:
            ...
            if not getattr(flags, "DOMAIN_PACKS", False):
                return
            if not hasattr(self, "_pack_signal_tasks"):
                self._pack_signal_tasks = set()
            task = asyncio.create_task(
                self._run_record_pack_signal(
                    completion_id=str(completion_id),
                    passed=(direction == 1),
                )
            )
            self._pack_signal_tasks.add(task)
            task.add_done_callback(self._pack_signal_tasks.discard)
        except Exception as e:
            logger.debug(f"_maybe_record_pack_signal failed: {e}")

    async def _run_record_pack_signal(self, *, completion_id: str, passed: bool) -> None:
        """Worker: own session, look up the completion's PackFireEvent, update the
        pack's win-rate. Self-contained; never raises."""
        from app.settings.database import create_async_session_factory

        async_session = create_async_session_factory()
        try:
            async with async_session() as session:
                from app.ai.packs.winrate import record_signal_for_completion
                await record_signal_for_completion(session, completion_id, passed)
        except Exception as e:
            logger.debug(f"_run_record_pack_signal failed: {e}")
