...
from app.schemas.data_sources.configs import (
    ...
    # Spreadsheet (uploaded Excel/CSV)
    SpreadsheetConfig,
    SpreadsheetNoAuthCredentials,
...
REGISTRY: Dict[str, DataSourceRegistryEntry] = {
    ...
    "spreadsheet": DataSourceRegistryEntry(
        type="spreadsheet",
        title="Spreadsheet (Excel/CSV)",
        description="Upload a local Excel (.xlsx/.xls) or CSV file and query it as a Data Agent. Each sheet becomes a queryable table via an in-memory DuckDB engine.",
        config_schema=SpreadsheetConfig,
        credentials_auth=AuthOptions(
            default="none",
            by_auth={
                "none": AuthVariant(
                    title="No Auth (uploaded file)",
                    schema=SpreadsheetNoAuthCredentials,
                    scopes=["system"],
                )
            },
        ),
        client_path="app.data_sources.clients.spreadsheet_client.SpreadsheetClient",
        # Un-gated, no license. Tables-shaped (sheets → SQL tables); the file is
        # uploaded by the org and stored server-side (single shared catalog).
    ...
    "spark_connect": DataSourceRegistryEntry(
        type="spark_connect",
        title="Spark",
        description="Run Spark SQL against a remote Spark cluster via Spark Connect (sc://). Compute runs on the cluster; DASH only sends SQL and receives results — no in-process engine on the DASH server.",
    ...
    "sharepoint": DataSourceRegistryEntry(
        ...
        data_shape="files",
        catalog_ownership="shared",
        # Hybrid: un-gated (file connector we operate). dash EE enforcement
        # (ENTERPRISE_DATASOURCES in app/ee/license.py) never listed sharepoint,
        # so this only drops the cosmetic badge; behavior unchanged.
        requires_license=None,
    ),
    "onedrive": DataSourceRegistryEntry(
        ...
        ui_form="integration",
        # Hybrid: un-gated file connector (not in EE ENTERPRISE_DATASOURCES);
        # drops cosmetic badge only.
        requires_license=None,
    ),
    ...
    "ms_fabric_user": DataSourceRegistryEntry(
        type="ms_fabric_user",
        title="Microsoft Fabric (User Login)",
        description="Microsoft Fabric Warehouse / Lakehouse SQL endpoint via your Entra email + password (ActiveDirectoryPassword). No app registration. Fails if your account has MFA / Conditional Access — use the Service Principal connector then.",
        config_schema=MSFabricUserConfig,
        credentials_auth=AuthOptions(
            default="userpass",
            by_auth={
                "userpass": AuthVariant(
                    title="Email / Password (Entra)",
                    schema=MSFabricUserCredentials,
                    scopes=["system", "user"],
                ),
            }
        ),
        client_path="app.data_sources.clients.ms_fabric_user_client.MsFabricUserClient",
        requires_license=None,  # Hybrid: all connectors un-gated
    ),
