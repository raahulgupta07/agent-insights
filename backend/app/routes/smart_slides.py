"""Smart Slide Deck Build (HYBRID_SMART_SLIDES) — additive sidecar.

Outputs "Generate slide deck" becomes smart: it understands the chat turn and the
agent's own data WITHOUT asking, and only asks ONE chip when there is no usable
signal. Two endpoints (both gated → {disabled:true} when flag OFF):

  GET  /api/reports/{id}/slides/context
  ...
  POST /api/reports/{id}/slides/smart-generate   body {prompt?, size?, depth?}
       Build the slide deck. Reuses report_slides._generate_artifact (mode='slides')
       with the user's steer prompt + size/depth folded in. If there are no charts
       AND no signal → returns needs_clarification with chip options instead of a 400.
...
router = APIRouter(tags=["smart-slides"])

# Asked ONLY when there is no usable signal (cold open + empty prompt).
_CLARIFY_QUESTION = "What should this slide deck help you communicate?"
_CLARIFY_OPTIONS = [
...
def _disabled():
    return {"disabled": True, "feature": "smart_slides"}
...
async def _resolve_clarify(db, organization, steer: str) -> dict:
    ...
    try:
        ...
        logger.debug("smart-slides: ambiguity gate unavailable, using static clarify")
...
async def _last_turn(db: AsyncSession, report_id: str) -> dict:
    ...
    try:
        ...
        logger.exception("smart-slides: last-turn read failed")
    return out
...
async def _agent_sources(db: AsyncSession, report: Report) -> List[dict]:
    ...
    try:
        ...
        logger.exception("smart-slides: agent-sources read failed")
    return out
...
async def _load_report(db: AsyncSession, report_id: str, organization) -> Optional[Report]:
    try:
        ...
        logger.exception("smart-slides: report load failed")
        return None


@router.get("/reports/{report_id}/slides/context")
async def smart_slides_context(
    report_id: str,
...
class SmartSlidesGenerateRequest(BaseModel):
...
@router.post("/reports/{report_id}/slides/smart-generate")
async def smart_slides_generate(
    report_id: str,
    body: SmartSlidesGenerateRequest = ...,
    current_user: User = Depends(current_user),
    db: AsyncSession = Depends(get_async_db),
    organization: Organization = Depends(get_current_organization),
):
    """Build the slide deck from the turn's existing charts, steered by the user's
    ...
    if not has_charts:
        return {"ok": False, "needs_data": True,
                "message": "Ask the agent a data question first so it builds the charts, "
                           "then generate the slide deck from them."}
    ...
    try:
        ...
        result = await _generate_artifact(
            mode="slides", report_id=report_id, current_user=current_user,
        ...
        logger.exception("smart-slides generate failed")
