...
async def get_reports(
    ...
    result = await report_service.get_reports(db, current_user, organization, page, limit, filter, search, scheduled, status, data_source_id, mode, has_artifacts)
...
# --- Dashboard Layout Versioning (snapshot a NEW version + restore) ---
# NOTE: no `from __future__ import annotations` in this module — a Body model on
# a permission-decorated route would otherwise be mis-read as a query param.

class LayoutSnapshotRequest(BaseModel):
    change_summary: Optional[str] = None
    source: Optional[str] = None  # 'chat' | 'manual' | 'autopilot' | 'restore'


@router.post("/reports/{report_id}/layouts/snapshot", response_model=DashboardLayoutVersionSchema)
@requires_permission('update_reports', model=Report, owner_only=True)
async def snapshot_layout(report_id: str, payload: LayoutSnapshotRequest = Body(default=LayoutSnapshotRequest()), current_user: User = Depends(current_user), db: AsyncSession = Depends(get_async_db), organization: Organization = Depends(get_current_organization)):
    return await layout_service.snapshot_active_layout(
        db,
        report_id,
...
@router.post("/reports/{report_id}/layouts/{layout_id}/restore", response_model=DashboardLayoutVersionSchema)
@requires_permission('update_reports', model=Report, owner_only=True)
async def restore_layout(report_id: str, layout_id: str, current_user: User = Depends(current_user), db: AsyncSession = Depends(get_async_db), organization: Organization = Depends(get_current_organization)):
    return await layout_service.restore_layout(db, report_id, layout_id, user=current_user)
