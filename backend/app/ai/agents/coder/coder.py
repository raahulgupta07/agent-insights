            ...
            try:
                ...
                if _pdflags.SANDBOX_PUSHDOWN:
                    pushdown_block = (
                        ...
                        "                - EXCEPTION FIRST: when the interpreted_prompt says \"return granular rows\" /\n"
                        "                  \"do not pre-aggregate\", follow that rule above — return one row per record,\n"
                        "                  no GROUP BY. This wins over everything below.\n"
                        ...
                        "                - **Uploaded files (`excel_files`) — DuckDB SQL is the DEFAULT path, not just a\n"
                        "                  fallback for big files.** When the run reads an uploaded file AND the question\n"
                        "                  is a summary / aggregate / top-N / count / filter (i.e. NOT \"return granular\n"
                        "                  rows\"), read AND aggregate the file with DuckDB SQL over `read_csv_auto('<path>')`\n"
                        "                  (or `read_parquet('<path>')`) and return the already-aggregated small result.\n"
                        "                  Do NOT `pd.read_csv` the whole file and then `groupby` in pandas.\n"
                        "                    import duckdb\n"
                        "                    df = duckdb.query(\n"
                        ...
                        "                    ).to_df()\n"
                        "                  (Substitute the real file path and the real column names from the schema above.)\n"
                        "                - ANTI-PATTERN: do NOT `SELECT *` (or read the whole file) and then filter /\n"
                        "                  aggregate in pandas when the answer is an aggregate — the database / DuckDB\n"
                        "                  must do the grouping, so only the small aggregated result is materialized.\n"
                    )
            except Exception:
                pushdown_block = ""
