...
class SchemaContextBuilder:
    ...
    async def build(
        ...
        *,
        ...
        # Studios (hybrid Studios ST2): when this report lives inside a studio,
        # the agent is grounded ONLY on the studio's pinned Data Agents. Restrict
        # the offered sources to that pinned set. Flag OFF or no studio_id ->
        # data_sources is left untouched (upstream-identical retrieval scope).
        data_sources_in_scope = await self._scope_to_studio(self.data_sources)
    ...
    async def _scope_to_studio(self, data_sources: List[DataSource]) -> List[DataSource]:
        """Restrict offered sources to a studio's pinned set (Studios ST2).

        When the report belongs to a studio AND flags.STUDIOS is ON, return only
        the DataSources pinned to that studio (via studio_data_sources). Flag OFF,
        no report, or no studio_id -> return data_sources unchanged so retrieval
        scope is byte-identical to upstream. Fails open (returns the input list)
        on any error so a studio mishap can never blank the schema context.
        """
        try:
            from app.settings.hybrid_flags import flags
            if not flags.STUDIOS:
                return data_sources
            studio_id = getattr(self.report, "studio_id", None) if self.report is not None else None
            if not studio_id:
                return data_sources
            ...
            pinned = set(str(a) for a in res.scalars().all())
            # No pins -> nothing in scope for a studio chat (don't leak the
            # report's other attached sources).
            return [ds for ds in data_sources if str(getattr(ds, "id", "")) in pinned]
        except Exception:
            return data_sources
