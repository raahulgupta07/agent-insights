...
- No new approval gate: this caches the agent's own validated output (unlike
  query_cache, whose proven SQL goes through the dash approval gate).
...
def _sources_fp(ids: Optional[list[str]]) -> Optional[str]:
    ...
    Returns a deterministic ``,``-joined sorted id string ONLY when the set has
    MORE THAN ONE source (a genuine multi-source Studio). Single-source or None
    -> None, so the cache key is left UNCHANGED for the common case (existing
    single-source rows keep hitting; zero cache invalidation, no migration).
    """
...
def _scoped_hash(
    ...
    sources_fp: Optional[str] = None,
) -> str:
    """Hash key for a normalized question, optionally namespaced to a Studio
    and to a pinned multi-source SET.

    Per-studio brain memory (hybrid Studios ST5) without a schema change: the
    ``answer_cache`` table has no ``studio_id`` column and migrations are barred
    here, so we scope by NAMESPACING the lookup key instead. When the Studios
    flag is ON and a ``studio_id`` is supplied, the cache key is derived from
    ``"studio:{id}\\n{norm}"`` — so a Studio chat only ever matches answers that
    were stored within the SAME Studio (its own learned memory), and a Studio's
    stored answers never collide with a non-studio chat's. The human-readable
    ``question_norm`` column still stores the real normalized question.

    ``sources_fp`` (multi-source set fingerprint, optional): when set (a Studio
    pinned to >1 data source), it is folded into the key so two Studios that
    differ ONLY in their pinned source SET don't mis-share a cached answer. The
    DB row filter still keys on the single primary ``data_source_id`` column —
    uniqueness for multi-source sets now comes from this hash. When None (single
    source or unpinned), the key is byte-identical to before (back-compat).

    ``studio_id`` None OR flags.STUDIOS OFF -> the studio namespacing is skipped.
    With both ``studio_id`` and ``sources_fp`` None -> plain ``question_hash(norm)``,
    byte-identical to upstream (no behavior change).
    """
    if studio_id:
        try:
            ...
            if flags.STUDIOS:
                if sources_fp:
                    return question_hash(f"studio:{studio_id}\n{sources_fp}\n{norm}")
                return question_hash(f"studio:{studio_id}\n{norm}")
        except Exception:
            # Defensive: any flag-read issue falls back to the un-namespaced key.
            pass
    if sources_fp:
...
async def serve_answer_cache(
    ...
    *,
    ...
    ``studio_id`` (hybrid Studios ST5, optional): when set AND flags.STUDIOS is
    ON, the lookup key is namespaced to the Studio so a Studio only serves its
    own learned answers (see ``_scoped_hash``). None / flag OFF -> unchanged.

    ``data_source_ids`` (multi-source set, optional): the FULL pinned source set.
    Only folded into the key when it has >1 source (see ``_sources_fp``), so two
    multi-source Studios with different pinned sets don't mis-share an answer.
    Single-source / None -> key unchanged (the ``data_source_id`` row filter is
    untouched; callers still pass the primary/first id).
    """
    from app.settings.hybrid_flags import flags
    ...
    if not norm:
        return None
    qhash = _scoped_hash(norm, studio_id, _sources_fp(data_source_ids))
...
async def store_answer(
    ...
    *,
    ...
    ``studio_id`` (hybrid Studios ST5, optional): when set AND flags.STUDIOS is
    ON, the stored row's hash key is namespaced to the Studio so its memory is
    private to that Studio (see ``_scoped_hash``). None / flag OFF -> unchanged.

    ``data_source_ids`` (multi-source set, optional): the FULL pinned source set,
    folded into the hash only when >1 source (see ``_sources_fp``) so multi-source
    Studios don't collide. Single-source / None -> key unchanged. The single
    ``data_source_id`` column still stores the primary/first id (no schema change).
    """
    from app.settings.hybrid_flags import flags
