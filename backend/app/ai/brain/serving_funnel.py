...
async def _try_answer_cache(
    ...
    *,
    ...
    ``studio_id`` (hybrid Studios ST5, optional): forwarded so a Studio chat
    only serves its own learned answers. None / flags.STUDIOS OFF -> unchanged.

    ``data_source_ids`` (multi-source set, optional): forwarded so a multi-source
    Studio keys its cache on the FULL pinned set (folded only when >1). None /
    single-source -> key unchanged.
    """
...
async def run_serving_funnel(
    ...
    *,
    ...
    ``studio_id`` (hybrid Studios ST5, optional): when the question is asked
    inside a Studio chat, forward the Studio id so Tier-① only serves that
    Studio's own learned answers. None / flags.STUDIOS OFF -> unchanged.

    ``data_source_ids`` (multi-source set, optional): the FULL pinned source set.
    Forwarded so the cache keys on the whole set (folded only when >1 source);
    multi-source Studios with different pinned sets won't mis-share answers.
    None / single-source -> cache key unchanged (back-compat).
    """
    # --- Tier ① answer-cache ------------------------------------------------ #
