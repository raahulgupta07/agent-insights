...
# --------------------------------------------------------------------------- #
# critic + bounded recursive verify (HYBRID_RECURSIVE)                         #
# --------------------------------------------------------------------------- #
_DEFAULT_VERIFY_RETRIES = 2   # extra attempts after the first (=> <=3 runs)
_MAX_VERIFY_RETRIES = 3       # hard ceiling — never loop more than this


async def critique_finding(*, model, question: str, result: dict) -> dict:
    """Grade ONE worker finding for HARD errors only. Never raises.

    Returns {"passed": bool, "reason": str, "hint": str}. FAIL-OPEN: any parse
    problem or empty critic reply -> passed=True (the critic must never block a
    finding it could not actually evaluate).
    """
    answer = str((result or {}).get("answer", "") or "").strip()
    sql = str((result or {}).get("sql", "") or "").strip()
    ok = bool((result or {}).get("ok", False))

    # Deterministic fast-fail: no answer at all is unambiguously bad.
    if not answer:
        return {
            "passed": False,
            "reason": "empty finding",
            "hint": "produce a concrete answer grounded in a SQL result",
        }

    prompt = (
        "You are a STRICT reviewer checking ONE research finding for HARD errors "
        "only. Fail ONLY when there is a concrete defect:\n"
        "- it says it could not answer / had no data / is not grounded in data\n"
        "- the SQL is empty or clearly does not match the question\n"
        "- the answer is for the wrong entity / time window / metric\n"
        "- it states numbers with no SQL backing them\n"
        "Do NOT fail for style, length, or taste. When in doubt, PASS.\n\n"
        f"Question: {question}\n"
        f"SQL used: {sql or '(none)'}\n"
        f"Grounded in data: {ok}\n"
        f"Finding: {answer[:1200]}\n\n"
        'Respond ONLY with JSON: {"verdict":"pass"|"fail","reason":"<short>",'
        '"hint":"<what to fix, only if fail>"}.'
    )
    raw = await _llm_infer(model, prompt)
    parsed = _extract_json(raw)
    if not isinstance(parsed, dict):
        return {"passed": True, "reason": "critic unparsed (fail-open)", "hint": ""}
    verdict = str(parsed.get("verdict") or parsed.get("status") or "").strip().lower()
    passed = (
        parsed.get("passed") is True
        or verdict in {"pass", "passed", "ok", "valid", "yes", "true", "accept"}
    )
    return {
        "passed": passed,
        "reason": str(parsed.get("reason", "") or "")[:300],
        "hint": str(parsed.get("hint", "") or "")[:300],
    }


async def run_subtask_verified(
    *,
    sub_question: str,
    model,
    ds_clients: Dict[str, Any],
    schema_hint: str = "",
    max_steps: int = 2,
    max_retries: int = _DEFAULT_VERIFY_RETRIES,
    critic_model=None,
) -> dict:
    """``run_subtask`` wrapped in a bounded critic-retry loop.

    Each attempt's finding is graded by ``critique_finding``; a HARD-error
    finding is re-run with the reviewer note appended, up to ``max_retries``
    extra attempts. Returns the same dict as ``run_subtask`` plus:
      - ``verified`` (bool): finding passed the critic
      - ``attempts`` (int): how many worker runs happened
      - ``critic_reason`` (str): last critic reason (why it failed, if unverified)

    Never raises. With no data clients there is nothing to re-query, so retries
    are skipped (one best-effort run only).
    """
    critic_model = critic_model or model
    try:
        max_retries = int(max_retries)
    except Exception:
        max_retries = _DEFAULT_VERIFY_RETRIES
    max_retries = max(0, min(max_retries, _MAX_VERIFY_RETRIES))
    if not ds_clients:
        max_retries = 0  # no data to re-query -> retrying is pure token burn
    ...
    while attempts <= max_retries:
        q = sub_question
        if note:
            q = f"{sub_question}\n(Reviewer note from the previous attempt: {note})"
        res = await run_subtask(
            sub_question=q,
            model=model,
        ...
        verdict = await critique_finding(
            model=critic_model, question=sub_question, result=res
        )
        last_reason = verdict.get("reason", "") or ""
        if bool(res.get("ok")) and verdict.get("passed", True):
            res["verified"] = True
            res["attempts"] = attempts
            res["critic_reason"] = last_reason
            return res
        # failed -> carry the reviewer hint into the next attempt
        note = verdict.get("hint") or verdict.get("reason") or res.get("error") or ""

    last = last or {"question": sub_question, "answer": "", "sql": "", "ok": False, "error": ""}
    last["verified"] = False
    last["attempts"] = attempts
    last["critic_reason"] = last_reason or last.get("error", "") or "did not pass review"
    return last
