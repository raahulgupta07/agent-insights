...
# Hybrid: emptied — this self-hosted fork un-gates ALL connectors (is_datasource_allowed
# returns True for every type). Restore the list to re-enable EE connector gating.
ENTERPRISE_DATASOURCES = []

# Features this self-hosted fork unlocks for EVERYONE regardless of license tier
...
# Fork un-gate: per-user ("user_required") connection auth is normally an EE-only
# capability. This self-hosted fork unlocks it for community too (mirrors the
# ENTERPRISE_DATASOURCES / COMMUNITY_FEATURES un-gates). Return is_enterprise_licensed()
# instead to restore the original gate.
_USER_AUTH_UNGATED = True


def is_user_auth_allowed() -> bool:
    """Whether 'user_required' connection auth policy is permitted."""
    return _USER_AUTH_UNGATED or is_enterprise_licensed()
...
def has_feature(feature: str) -> bool:
    ...
    """
    # Fork un-gate: these features are free for all tiers (see COMMUNITY_FEATURES).
...
def require_enterprise(feature: Optional[str] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            # Fork un-gate: community-unlocked features skip the license gate entirely.
            if feature in COMMUNITY_FEATURES:
