...
icon: 'server'
description: 'Install Dash on your own server using Docker or Kubernetes'
---

## Install with Docker
---

You can install Dash with a single docker command. By default, it will use SQLite as the database.
You can also configure it to use PostgreSQL by passing `DASH_DATABASE_URL` environment variable.
...
For example: `DASH_DATABASE_URL=postgresql://postgres:postgres@localhost:5432/dash`
</Info>
...
Run Dash with Docker Compose and Caddy (built-in TLS on port 443). We recommend using the canonical files from the repo to avoid drift:
...
You can also configure additional settings in the dash-config.yaml file.
...
```bash
docker run --pull always -d -p 3000:3000 -v $(pwd)/dash-config.yaml:/app/dash-config.yaml bagofwords/bagofwords
```
...
helm upgrade -i --create-namespace \
 -ndashapp-1 dashapp dash/bagofwords \
...
```bash
# Restart the Dash deployment(s) to pick up the latest image
# Adjust namespace (-n) and selector if you used different names
kubectl rollout restart deployment -n dashapp-1 -l app.kubernetes.io/instance=dashapp
kubectl rollout status deployment -n dashapp-1 -l app.kubernetes.io/instance=dashapp
```

## AWS Aurora Configuration
---

Dash supports connecting to **AWS Aurora PostgreSQL** using IAM database authentication. This eliminates static database passwords entirely — the application generates short-lived tokens (valid for 15 minutes) at connection time using AWS IAM.
...
```sql
CREATE USER dash_user;
GRANT rds_iam TO dash_user;
GRANT ALL PRIVILEGES ON DATABASE postgres TO dash_user;
```
...
eksctl create iamserviceaccount \
  ...
  --attach-policy-arn arn:aws:iam::<ACCOUNT>:policy/dash-rds-connect \
  --approve
```
...
### dash-config.yaml for Aurora (non-Kubernetes)

If you are running Dash on EC2 or ECS (not Kubernetes), you can configure Aurora IAM auth directly in `dash-config.yaml`:
...
To enable Google OAuth authentication, configure the following parameters in your dash config (or in env/k8s configmap):
...
2. Set callback URL `https://your-base-dash-url.com/api/auth/okta/callback`
