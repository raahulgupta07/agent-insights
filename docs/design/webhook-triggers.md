...
1. Sender POSTs the event → Dash verifies the HMAC signature (rejects `401` if it
...
- The existing `ApiKey` (`dash_…`) authenticates the *whole* Dash API as a
...
- **Generic sources also use HMAC.** GitHub verifies via its native
  ...
  plus `X-DASH-Timestamp` (rejected if skew > 5 min → replay guard) and an
