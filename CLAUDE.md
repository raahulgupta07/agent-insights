...
**Current state (2026-07-03):** image `cityagent-analytics:dev` on `:3007` (baked `:v1.87.0` (rebuilt 2026-07-03, image carries v1.85-1.87 + mig agentknow1)), branch
**`feature/table-relevance-overview`** (PUSHED `origin/feature/table-relevance-overview`; PR to `dev` = user's click, gh not authed; `main` untouched per branch-flow), mig head **`colprofile1`**,
`VERSION_HYBRID`=**1.83.0**. Superuser THIS DB = `demo@test.com` / `CityAgent#2026` (NOT admin@cityagent.io — that email not on this DB).
