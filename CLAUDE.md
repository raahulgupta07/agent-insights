...
**Current state (2026-07-07, NEWEST — v1.160.4 BAKED BE + recreated healthy):** `VERSION_HYBRID`=**1.160.4**, remote `agent-insights` `main` (v1.160.3 was `f7fc274d`, v1.160.2 `e57e5ed9`). Rollback imgs `pre-v1604`/`pre-v1603`/`pre-v1602`/`pre-v1601`/`pre-v160`. Org `b2bec83d` ("Main Org", ONLY org), admin `admin@cityagent.io`/`Admin12345`.
- **v1.160.4 — NPM/HTTPS deploy fix + clean multi-worker boot (BAKED into `:dev`, recreated healthy):** ★"insights UI didn't enable behind Nginx Proxy Manager" root cause: `docker-compose.npm.yaml` shipped with a DIFFERENT identity than the running build stack — `dash-app`/`dash-postgres`/`dash-network`/`postgres_data` vs live `ca-app`/`ca-postgres`/`ca-network`/`ca_*`. NPM on `dash-network` can't resolve `ca-app` → 502; whole-file up = different volumes = empty DB. FIX: rewrote `docker-compose.npm.yaml` as a drop-in SUPERSET of build.yaml (identical `ca-*` names/`ca-network`/`ca_*` vols/full env) + `nginx-proxy-manager` (`ca-npm`) on ca-network. ★NPM proxy host: Forward Hostname=`ca-app`, Port=**3000** (app internal port, NOT 8077 like Aria), Websockets ON, advanced `proxy_buffering off` (chat SSE). (2) `start.sh` uvicorn +`--proxy-headers --forwarded-allow-ips='*'` (was missing → wrong scheme behind HTTPS proxy → secure-cookie login/wss break; Aria had it). (3) ★multi-worker boot race: `scheduler.start()` (main.py:758) runs in ALL workers (non-leaders persist user jobs) → fresh-DB `create_all` race on `apscheduler_jobs` → `duplicate key pg_type_typname_nsp_index` → "Application startup failed. Exiting" (self-heals, 502s meanwhile). FIX: NEW `scheduler.ensure_jobstore_table()` (`CREATE TABLE IF NOT EXISTS`, exact APScheduler schema, fail-soft) run ONCE pre-fork in start.sh (`python -m app.core.scheduler`). Verified: proxy-headers active in running uvicorn, pre-create idempotent (returns True, table exists), boot clean no dup-key. Detail → [[project_cityagent_wren_julius_optimization]].
- **v1.160.3 — PBI table discovery: real schema via `INFO.VIEW.COLUMNS()`, brute-storm killed AT SOURCE (BAKED into `:dev`, recreated healthy):** v1.160.2 cooldown only *absorbed* the 429; this removes the cause. ★Root cause the cooldown missed: some datasets return an EMPTY `COLUMNSTATISTICS()` **without erroring** (`stats_df.empty` → `empty_db=False`) so they slip past the empty-db short-circuit → fall to `_brute_discover_tables` → 40 guessed names → 429. Live culprit `EDA_Model_Toey` (COLUMNSTATISTICS→0 rows). FIX (`powerbi_client.py`): NEW `_get_tables_via_info_view` runs `EVALUATE INFO.VIEW.COLUMNS()` (same DMV family as `fetch_model_metadata` rels/measures; delegated-user friendly, unlike bare `INFO.TABLES()` which 400s over executeQueries) → real table names + real column dataTypes in ONE call, filters `DateTableTemplate_*`/`LocalDateTable_*` + `IsHidden` cols. Wired into `get_dataset_tables` BETWEEN empty-db check and REST/brute; brute now absolute-last-resort. NOT flag-gated, fail-soft (any error → exact prior behavior). ★LANDMINE: `INFO.VIEW.COLUMNS`/`.TABLES`/`.RELATIONSHIPS`/`.MEASURES` WORK over PBI executeQueries; bare `INFO.TABLES()`/`INFO.COLUMNS()` 400. Live-verified org b2bec83d: `get_dataset_tables(EDA_Model_Toey)` → 6 real tables/980ms/**0** "Cannot find table" warnings (was ~40); INFO.VIEW.COLUMNS OK on every non-empty dataset (76/151/175 cols). Detail → [[project_cityagent_wren_julius_optimization]].
- **v1.160.2 — PBI dashboard stall + Stop-button 500 FIXED (BAKED into `:dev`, recreated healthy):** (1) ★**PBI 429 storm** = `_brute_discover_tables` (`powerbi_client.py`) sprays ~40 generic English table-name guesses (`Categories/Suppliers/Region/Measures/Table`…) at EVERY dataset that rejects COLUMNSTATISTICS → 174 doomed probes/15min → trips PBI 120-req/min/user cap (429) → real dashboard query throttled ~60s → UI parks "Starting… 0/0" then recovers. FIX = persistent client-wide 429 cooldown (`_dax_cooldown_until`): first throttle stamps it, subsequent DAX short-circuits with a `429`-tagged `PowerBIRateLimitError` (msg keeps "429" so brute abort-Event still trips) instead of re-hammering. Gated on `CONNECTOR_ROBUSTNESS` (ON). Set in `_post_dax_with_retry` on any 429, checked at top of `_execute_dax_internal`. (2) **Stop button 500** = `completion_service.update_completion_sigkill` returned raw ORM `Completion` → FastAPI serialized relationship graph (completion→report→completions→…) → `RecursionError`. FIX = return plain dict `{ok,id,status,sigkill}` (kill stamp already landed pre-fix; only the response was broken). ★LANDMINE: never return a raw ORM object with loaded relationships from a route — serialization recurses → 500. Detail → [[project_cityagent_wren_julius_optimization]].
- **WREN/JULIUS OPTIMIZATION — Phases 0-4 + 3b ALL SHIPPED (v1.156-1.160.1), speed + accuracy + Julius-quality output. Detail → memory [[project_cityagent_wren_julius_optimization]].** Investigation "why slow/OOM on AWS vs WrenAI+Julius": app ran LLM pandas IN-PROCESS (mem accumulates→OOM restart loop) + aggregations in pandas not SQL. Fork already had both lanes built but OFF + a config gap.
  - **Phase 1 (v1.156) Julius lane SAFE:** `subprocess_pool.py` `_ChildQueryCapture` wraps live clients IN CHILD (read-only guard + capture sql/ms/rows/bytes); `code_execution._replay_subprocess_quota`. Concurrency env `HYBRID_SANDBOX_MAX_CONCURRENCY`.
  - **Phase 2 (v1.157) Wren lane:** `coder.py` pushdown→DuckDB-over-file DEFAULT for upload aggregates (`duckdb.query(read_csv_auto)` GROUP BY not pandas). NEW `sql_validate.py` (sqlglot parse→read-only+shape, `build_sql_error_feedback`) in BOTH retry loops. `sqlglot==30.7.0`. Flag `HYBRID_SQL_VALIDATE`.
  - **★v1.157.1 LANDMINE RLIMIT_AS false-trips DuckDB:** `DEFAULT_SANDBOX_MEM_LIMIT_MB=2048` as RLIMIT_AS (VIRTUAL addr space) → DuckDB/pyarrow reserve >2GB virtual on tiny data → false "OutOfMemoryException" on GROUP BY. FIX `_resolve_subprocess_mem_cap_mb()` default **0 (no RLIMIT_AS)**; real cap = cgroup `mem_limit` + one-shot reclaim + timeout + semaphore. RULE: NEVER RLIMIT_AS-cap analytical code (duckdb/pandas/arrow); use cgroup RSS.
  - **Phase 3 (v1.158) coder grounding:** NEW `ai/agents/coder/grounding.py::build_grounding_context` reuses approved Semantic/Metrics/JoinGraph builders, scoped to target tables, injected before pushdown "use joins/metrics EXACTLY". `CodeGenContext.grounding_context` seam. Flag `HYBRID_CODER_GROUNDING`. LANDMINE: joins come from `TableEdge` (JOIN_GRAPH) not semantic layer.
  - **Phase 3b (v1.159) paraphrase recall:** `query_learning.recall_learned_queries` Jaccard→embedding+RRF fusion (`_rrf_rerank` k=60, golden tiebreak, cosine-floor 0.35). NO embedding col on `query_library_items`→vectors computed on-fly, 1 batched `embeddings.embed_texts`. Fail-soft→Jaccard. Flag `HYBRID_RECALL_RRF`.
  - **Phase 4 (v1.160) Julius-quality output** (3 disjoint subagents): NEW `ai/knowledge/notebook.py::build_analysis_notebook` step-by-step show-your-work from Steps (**Step has NO report_id→join `Widget.report_id`**), endpoint `GET /api/reports/{id}/notebook` on `report_slides.py` + **v1.160.1 FE `components/report/ReportAnalysisNotebook.vue`** mounted under `<ChatSummary/>` (explicit import); `followups.py` data-aware (`_recent_result_columns` pulls latest tabular step cols); `create_data._smart_viz_override` +4 rules (0rows→table/1val→metric_card/date→line/2dims→heatmap, each checks ALLOWED_VIZ_TYPES); `data_source.generate_conversation_starters` injects sampled `values=`. Flags `HYBRID_ANALYSIS_NOTEBOOK`/`HYBRID_FOLLOWUPS_DATA_AWARE`/`HYBRID_STARTERS_DATA_GROUNDED` + SMART_VIZ broadened.
  - **ALL 9 flags ENABLED org b2bec83d** (DB `hybrid_overrides`, survive force-recreate—loaded at boot, NOT env). E2E-proven no-errors: data-aware followups cite real col+value ("higher than 1.54?"), notebook builds+renders. Phase 0 AWS config (`docker-compose.npm.yaml` mem_limit + `AUTOTRAIN_STAGING_ROLE_SECRET` passthrough, `docs/AWS_PHASE0_RUNBOOK.md`, `scripts/aws_phase0.sh`) — **PENDING engineer deploy** (root-cause = env var unset → staging DEAD → dlt/DuckDB single-writer collision). ★LANDMINES: `import main`→`app.routes`=3 (lazy app)→verify routes via HTTP 401 not route-list; `flag_modified(st,"config")` after mutating hybrid_overrides JSON col; zsh no word-split unquoted `$var` (use arrays).

**Prior state (2026-07-06 — v1.145.0 BAKED + recreated):** `VERSION_HYBRID` was **1.145.0**. Rollback img `cityagent-analytics:pre-v145`.
- **v1.145 — DATA-AGENT TRAINING UX: header Train button + Auto-train toggle + always-visible training-flow + live CLI.** Header **Train** (push-to-train, confirm popover, model=`default_data_train_model_id`=gemini-3.5-flash) + **Auto-train** toggle (default ON) in `layouts/data.vue`. **Auto-train-on-sync:** `per_user_connector.sync_clone_bg` runs training after sync (cadence 'on_sync'). NEW `services/connector_auto_train.py` (`run_autotrain_and_log`/`train_bg`/`_approve_proposed`, resolves data-train model). ★LANDMINE reconfirmed: **`data_sources` has NO `config` column** (moved to `Connection`; `data_source.config['bi_mode']` silently no-ops) → per-agent auto-train config stored in `organization_settings.config['connector_auto_train'][ds_id]` `{enabled,cadence,auto_approve}` (same pattern as `connector_auto_sync`). Endpoints `GET/PUT /data_sources/{id}/auto-train` + `POST /data_sources/{id}/train` (busy-guard via non-terminal `connector_sync.get_run` phase). **Stage tags:** `connector_sync.log_step` gained `stage`; `get_run` returns `stages{}` (9: discover·introspect·profile·values·meanings·metrics·queries·rules·ready, latest-status-per-stage, unseen=queued/skipped) + `learned{}` (tables/columns/definitions/metrics/values/rules, fail-soft). **FE:** NEW `components/agents/AgentTrainingFlow.vue` = always-visible 9-node strip (equal-flex FIT, NO L-R scroll — `flex:1 1 0` nodes + shrinking arrows, `overflow:hidden`) + live stage-tagged CLI terminal (reads `sync-status.log[].stage`, auto-scroll, warm-dark) + "what got learned" lanes; mounted on `pages/agents/[id]/index.vue` Overview under the synced banner; `AgentRobotDock` click scrolls to `#agent-training-flow`. Backups `backend/.autotrain_backup/` + `backend/.trainflow_backup/` + `frontend/.trainflow_backup/`.
- **v1.144 — SPLIT TRAINING MODEL (Data Agent vs Studio), both = `google/gemini-3.5-flash`.** Settings→LLM Agent Defaults now has FOUR pickers: Analysis / **Data Agent training** / **Studio Agent training** / Router-Small. Backend org-default keys in `organization_settings.config`: NEW `default_data_train_model_id` + `default_studio_train_model_id` (shared `default_train_model_id` kept as fallback). `_DEFAULTS_KEYS` in `routes/llm.py` (GET/PUT auto-cover new keys). **BUG FIXED:** data-agent autotrain (`routes/autotrain_connector.py`) was hardcoded to `is_default` (=analysis model) → now resolves `default_data_train_model_id → default_train_model_id → is_default`. Studio training (`ai/knowledge/train_orchestrator.py`, BOTH resolvers ~L302 + ~L643) prefers `default_studio_train_model_id → default_train_model_id`. FE `pages/settings/models.vue` single Training dropdown → two (4-col grid; load falls back to shared `train`, save writes both + syncs `train`). **Connector autotrain `_IDENT` guard relaxed** `^[A-Za-z0-9_ ./\-]+$` (was `^[A-Za-z0-9_]+$`) so PBI slash/space table names train (`app/services/autotrain/connector.py:22`). Current DB defaults org b2bec83d: analysis=sonnet-4.6, data_train=studio_train=gemini-3.5-flash, router=gemini-3.1-flash-lite; `is_default`=sonnet-4.6, `is_small_default`=gemini-3.1-flash-lite. Backup `scratchpad/pretrain_20260706_193323/`.
- **MODEL BAKE-OFF (gemini-3.5-flash vs sonnet-4.6, same description-gen prompt, 3 PBI tables):** gemini-3.5-flash = same accuracy/0-halluc, **~2.7× faster** (1.85s vs 5.0s/table), **~10× cheaper**; sonnet slightly richer business framing (at-risk/open-project). VERDICT (validated current design): **train→gemini-3.5-flash, live analysis→sonnet-4.6**. LANDMINE: `POST /api/autotrain/from-connector` (`flags.AUTOTRAIN`) on an ALREADY-TRAINED agent = near-no-op (semantics dedupe→0 kept; metrics `_build_metrics` are HEURISTIC not LLM + flushed-not-committed→0 persisted; qa=0 bc PBI≠SQL). Run head-to-head via `import main` (registers mappers) + `LLM(model_row).inference(prompt)` inside ca-app; usage async-record may not flush on plain `python` exit.
- **DATA-AGENT TAB FILL (org b2bec83d PBI agent DS `325cee90`, data not code):** Queries tab (0 entities→**13 metric entities** published+approved from metric_definitions, skipped UI-junk); example values (**85 dim columns** enriched via live DAX `EVALUATE TOPN(50,VALUES())` into `datasource_tables.columns[].metadata['values']` — THE agent example-value channel, NOT semantic_columns which has no such field); Instructions (**+5 grounding rules** measure-names/relationships/live-DAX, scoped via `instruction_data_source_association`, NOT global). Backup `scratchpad/dbbackup_20260706_190356/`.
- **SPEED PLAN (Julius-inspired) — Phase 1+2 BAKED, all flags default OFF (byte-identical until enabled).** Three data lanes: LOCAL (uploads/Studio in PG = already fast) · WAREHOUSE (SQL pushdown) · BI (Power BI/Fabric live-DAX = slow → snapshot). New `app/services/speed/`: `source_lane.py` (classify LOCAL/WAREHOUSE/BI), `session_cache.py` (warm client+schema cache, gated hook in `construct_clients` AFTER the access guard), `bi_snapshot.py` (snapshot BI tables → local DuckDB), `snapshot_client.py` (SQL-over-DuckDB client, rejects DAX). Flags `HYBRID_FAST_LANE`(master)/`WARM_SESSION`/`PLANNER_COLLAPSE`/`BI_SNAPSHOT` (Performance category, all OFF). **v1.140 BI router**: `construct_clients` gained `prefer_live` kwarg + a gated router that swaps a live BI client → `SnapshotClient` when a fresh snapshot exists (+`_bi_live_mode()` per-agent toggle `data_source.config['bi_mode']='live'`); `per_user_connector.sync_clone_bg` snapshots on sync. ★LANDMINE: the BI client's offline table-index was EMPTY on the multi-dataset PBI connector → name→dataset resolution = (None,None) → brute-name probe → all pulls failed; FIX = pass exact `datasetId`/`workspaceId` per table EXPLICITLY (from `datasource_tables.metadata_json['powerbi']`) to `execute_query`, skip `queryable=false`. VERIFIED LIVE: 200 rows→DuckDB (no probe), `SELECT COUNT(*)` = **9ms vs live-DAX seconds**. Snapshot pull itself is slow (~40s/table live, one-time background). NEXT: Phase 3 (planner collapse/warehouse pushdown/fast codegen model) + Phase 4 (Julius quality: smart viz/notebooks/data-driven starters). Rollback imgs `pre-v1139`/`pre-v1140`. Backups `backend/.backups/{1783322529_speedplan_phase1,1783324144_speedplan_phase2}/`. Org `b2bec83d` ("Main Org", ONLY org), admin `admin@cityagent.io`/`Admin12345`. Rollback images `pre-v1135`/`pre-v1136`/`pre-v1137`/`pre-v1138`. Session backups `backend/.backups/{1783318496_instrscope_automodel,1783321168_dbclients_keyfix}/` + `frontend/.backups/1783317133_uxpbi/` + `backend/.backups/*_hybridflags_autoartifact.py`.
- **v1.138.0 — "Execution error: 'powerbi'" KILLED permanently (all connectors).** ROOT: `data_source_service.construct_clients` keys `db_clients` by `"{ds_name}:{conn_name}"` + bare `"{ds_name}"` alias — NEVER a connector-type literal. Client docstring examples hardcoded `db_clients['powerbi']`/`['sisense']` → weak models copy verbatim → `KeyError: 'powerbi'` → first attempt fails + burns a recovery turn (misread as "connection error"). FIX 3 layers (connector-agnostic, additive, exact-key path byte-identical): (1) **`ClientResolver(dict)`** in `ai/code_execution/code_execution.py` wraps `db_clients` at `execute_code`/`execute_and_update_step` chokepoint → resolve exact→normalized(case/space/`:conn`-strip)→type-alias-from-CLASS-NAME(`PowerBIClient`→powerbi/pbi, `MsFabricClient`→fabric, `SisenseClient`→sisense; future connectors self-alias)→single-client-fallback→else clear `KeyError` listing valid keys. Every tool + future connector inherits it (single chokepoint). (2) `create_data.py` injects "AVAILABLE DATA CLIENTS" block (real keys, shortest/bare per client, fail-soft) into code-gen prompt. (3) `powerbi_client.py`+`sisense_client.py` examples → `db_clients[<your data source name>]` + "don't use connector-type name" note. Verified vs real `PowerBIClient`: `['powerbi']`/`['pbi']`/real-name/single-fallback resolve; 2-source bad key → clear error. ★LANDMINE: PBI client class = `PowerBIClient` (NOT `PowerBIUserClient`). RULE: never teach a literal `db_clients['<type>']` in a client docstring — key is the DS NAME; resolver now tolerates but prompt should list real keys.
- **v1.137.0 — auto-dashboard RETIRED (user-initiated only).** `HYBRID_AUTO_ARTIFACT` was silently building a 6-widget dashboard in the BACKGROUND after every data turn with no artifact (`services/auto_artifact.py::schedule_auto_artifact`, hooked in `completion_service.py` non-stream ~1101 + stream ~2706). ROOT: env default was **True** despite a stale "Default OFF" comment. Fix: default→**False** (`hybrid_flags.py:534 _bool("HYBRID_AUTO_ARTIFACT", False)`), meta `status:"legacy"` + `superseded_by:["HYBRID_ONECLICK_ARTIFACTS"]` (→ Features Legacy tab), org override false. Dashboards now build ONLY via chat ask or the "Build a dashboard" modal (`HYBRID_ONECLICK_ARTIFACTS`, unchanged).
- **v1.136.0 — instruction cross-agent LEAK killed + auto-model safety floor + Gemini small model.** (1) **Leak:** a Power BI agent showed "You are the MM Conso CRM analyst…". ROOT: instructions with ZERO `instruction_data_source_association` rows are treated ORG-GLOBAL and force-injected (`load_mode="always"`) into EVERY agent (`ai/context/builders/instruction_context_builder.py:187` — legacy mode `unscoped=global`). Fix = (data) linked 22 orphan MM Conso/data-quality instructions → MM Conso DS `de1dabd8` + enabled **`HYBRID_SCOPED_INSTRUCTIONS`** org override (unscoped≠global unless explicit `global_status`); (code) `services/train/definition_instructions.py:93` now binds each `AgentDefinition` instruction to `d.data_source_id or data_source_id` (was: linked ALL org-wide defs to whatever source trained now → A got B's). VERIFIED via real `InstructionContextBuilder.load_always_instructions`: PBI loads 0 CRM, MM Conso keeps 7. NOTE PBI's own "two distinct business domains" overview is REAL (10/10 tables match) not hallucination. (2) **Auto-model floor** (`ai/knowledge/auto_model.py::capability_floor(connector_kinds,table_count,question)` folded into `choose_auto_model` min_tier via stronger-of; wired from `completion_service.py::_report_capability_signals` cheap indexed joins): PBI/DAX + joins → never below BALANCED, forecast/why/dashboard → REASON. **`HYBRID_AUTO_MODEL` still OFF** (groundwork; enable only after golden-eval). (3) `llm_models.is_small_default` flipped Haiku 4.5 → **`google/gemini-3.1-flash-lite`** (drives follow-ups + sense-making + tie-break; data-only). ★LANDMINE: unscoped published instruction = org-global leak in legacy mode; every per-source instruction MUST link `inst.data_sources=[ds]` after flush (agent_overview.py pattern) — audit found only definition_instructions.py broken, rest already link or are studio-scoped/intentional-global.
- **v1.135.0 — chat UX + PBI grounding (4 subagents).** (FE `reports/[id]/index.vue`) `showChatThinkingSkeleton` filters `source_type==='plan'` blocks before the has-content check → shimmer stays up while agent plans (plan block renders only in right-panel Progress); follow-up loading = named row + `.cai-sk-chip` shimmer. (FE `PromptBoxV2.vue::selectedModelLabel`) never "Select Model" — Auto→`Auto · <autoPicked|default>`, stale/unset→org default/first, empty→"Loading model…". (BE `ai/knowledge/followups.py::_build_prompt`) trimmed to question+answer+table-names (dropped voice/instructions/schema-dump + 3 DB round-trips), asks EXACTLY 3 ≤90-char Qs. (BE PBI grounding) `powerbi_client.py::_grounding_rules_prompt`/`_real_table_names` (from cached offline `_table_index`) into `system_prompt()`; `create_data.py` appends PBI HARD-RULE block guarded on `isinstance(PowerBIClient)` in `ds_clients` → any model uses REAL table names, `'Table'[Col]` refs, no generic Table/Data/Measures/Dimension, no `id` col. Kills "Cannot find table 'Dimension'"/"Execution error: 'id'".

**Prior state (2026-07-06):** `VERSION_HYBRID`=**1.129.0** on branch `integration/merge-all-20260705`. v1.126–1.129 = this session's ships — **BAKED into `cityagent-analytics:dev` (verified code-in-image), git-COMMITTED (`3ab25573` code + `10dbbd39` infra docs) AND PUSHED to a NEW repo.** Org `b2bec83d` ("Main Org", ONLY org), admin `admin@cityagent.io`/`Admin12345`.
- **NEW GitHub repo `agent-insights`:** remote `agent-insights` = `git@github.com:raahulgupta07/agent-insights.git`, pushed to **`main`** (SSH auth, `gh` NOT logged in — used SSH). This is SEPARATE from `origin` (=`raahulgupta07/rahulai-dash`, untouched) + `upstream` (=bagofwords). Local branch `integration/merge-all-20260705` now TRACKS `agent-insights/main` (so a bare `git push` from here goes to agent-insights, NOT origin — re-point if you meant origin). SECURITY: before first push, untracked `frontend/localhost-key.pem` (a real dev PRIVATE key) + `localhost.pem` via `git rm --cached` + `.gitignore` (`.env*` was already ignored; no live secrets in tree). `.gitignore` now also ignores `*.bak.*` (session backups). NEVER push to `main` on ORIGIN/upstream; agent-insights is the user's own new repo.
- **INFRA SIZING doc** (`docs/INFRA_SIZING.md` + README section): 500 users / 50% concurrent (~120 concurrent in-flight queries) → ~26 vCPU / ~110 GB + ~1 TB SSD (4× app 4vCPU/16GB=16 workers, PG18+pgvector 8vCPU/32GB 500GB-1TB, PgBouncer, Redis 4-8GB, S3/MinIO). **LLM=OpenRouter external → NO GPU.** Uploads land in PG warehouse (main storage driver; cap ingests, DuckDB single-writer); connectors query external systems (compute is there, local=egress+ODBC+paced sync).
- **v1.129.0** (2026-07-06): chat perceived-latency + follow-up speed. (FE `reports/[id]/index.vue`) `showChatThinkingSkeleton(m)` shimmer card fills the pre-first-token gap (local `.cai-sk-*` clay-gradient, replicated from DashboardSkeleton; `stream-caret` v-if += `&& !showChatThinkingSkeleton(m)` kills the orphan blink); ACTIVITY `activityNowLabel` = `activityNow || (runActive?'Starting…':'Idle')` (no more "Idle" while working) + `plansWarming` shimmer; Working Folders "no tables" badge via `loadDsTableCounts()` (`/data_sources/active?include_unconnected=true` → `id→table_count`; report GET's `table_count` is a default-0, unreliable) + `coworkDsEmpty` (confirmed-0 only). (BE `agent_v2.py::main_execution`) flag **`HYBRID_FOLLOWUP_FASTPATH`** (3-place, default OFF, ON org b2bec83d via `hybrid_overrides`) appends a "FOLLOW-UP FAST PATH" hint to `PlannerInput.instructions` when the report already ran `read_resources`/`describe_tables` earlier (reuses the `ToolExecution ⋈ AgentExecution` query) → planner skips the redundant re-read turn; fail-soft, byte-identical OFF. Serving funnel keys on exact question+DS so novel follow-ups can't cache — this skips the RE-READ, not the answer.
- **v1.128.0**: agent plan JSON → checklist card. (FE `reports/[id]/index.vue`) `blockPlanTasks(block,msg)` tolerantly extracts `{"tasks":[…]}` from the thinking block's `plan_decision?.reasoning||reasoning||content` (mirrors `extractPlanTasks` map + `activityPlan` status; NOT reused because `extractPlanTasks` only reads `source_type==='plan'` `content`, not `reasoning`) → `.plan-card` checklist (✓/spin/○) else markdown fallback. Caret (`stream-caret`, `ca-blink` in `assets/css/transitions.css:69`) gated `+= m.status==='in_progress'` (was `!isBlockFinalized` which never flips for plan blocks → blinked forever).
- **v1.127.0–.2**: **Connector Reports** tab. (Settings `connectors.vue` 2-tab wrap: Connectors | Connector Reports; new `components/connectors/ConnectorsReports.vue`.) 2 admin-gated endpoints on `routes/data_source.py` + new `services/connector_report.py` (org-scoped, fail-soft): `GET /api/connectors/report?days=` → per-connector groups (all catalog connectors incl 0-user; `connected_count/active_7d/questions/tokens/cost_usd/users[]`), `GET /api/connectors/report/{ds_id}?days=` → per-user detail (top_questions/daily_questions 14d/sync_history/tokens_in-out/avg_latency). Data map: per-user clone = `DataSource(owner_user_id,template_source_id,is_user_template=False)`; email via User join; **`connector_email`** (real MS/PBI sign-in acct) via `_conn_account_email` reading clone `connection.config` (`email`/`ms_account_email`/`upn`/…) → name-suffix fallback; questions from `Completion role='user'` via `report_data_source_association`; tokens/cost from `LLMUsageRecord` grouped `(user_id,data_source_id)`; tables from `ConnectorSyncRun.tables_total`. **v1.127.2** browser tab title: `nuxt.config.ts app.head.title='City Agent Insights'` (was showing the URL — no default title).
- **v1.126.0–.1**: non-admin nav (Manage+Settings super-admin-only, `useAppNav.ts superAdminOnly`) + LDAP `preview_sync` fix (`_get_org_user_map` in `ee/ldap/sync_service.py`). LDAP verified WORKING (9/9). Isolation test PASS (per-user PBI clones + studios no-leak, `requires_resource_permission` 4-tier).
- **PENDING:** git commit (feature branch, NOT main) + bake all of v1.126–1.129 into `cityagent-analytics:dev`. Backups this session: `reports/[id]/index.vue.bak.*`, `settings/connectors.vue.bak.*`, `routes/data_source.py.bak.*`, `hybrid_flags.py.bak.followup1`, `agent_v2.py.bak.followup1`.
- **v1.125.1** (`f35301b1`): **Fix — clicking a report bounced to a blank page.** v1.122's `onAgentPickerChange` (bound to composer picker `@update:selectedDataSources` at `pages/reports/[id]/index.vue:753`) fired on the picker's INITIAL emit during report LOAD; the composer defaults to the global `useAgent` selection (≠ the opened report's persisted `data_sources`) → with chat-history + `!sameSet` it ran `navigateTo('/reports/new')` = dumped user on a blank scratch draft. Fix (FE-only, `frontend/pages/reports/[id]/index.vue`): (1) `agentSwitchArmed` ref armed 1.5s after `report.data_sources` settles (immediate watcher ~L2216 + `_armTimer`); (2) early-return guard in `onAgentPickerChange` — `if (!agentSwitchArmed.value){ currentAgents.value=val; return }`. Load-time emits ignored; only a real user pick switches/creates. LANDMINE: composer picker emits programmatically on mount — any nav-on-change handler must gate on a settle/arm flag.
- **v1.125.0**: **Blank reports no longer saved / shown.** `/reports/new` eagerly created a draft on every visit + no sidebar filter + v1.122 isReusable multiplied orphans → many empty "untitled report" rows. 3-part fix: (a) `report_service.get_reports` (~L1207) filters out placeholder-titled reports with 0 completions (`_placeholder_titles` OR `Report.id.in_(Completion.report_id)`); (b) `reports/new.vue` `isReusable` relaxed to reuse ANY existing 0-completion draft + new `rescopeDraft(id)` re-points the ONE scratch at the picked agent (kills per-switch orphans); (c) cleaned 22 existing blank drafts (soft-archive). Report auto-title stays placeholder until first user msg (`completion_service.py:669-686`); DELETE /reports/{id}=SOFT delete (status='archived', sidebar filters archived).
