...
**Current state (2026-07-04, LATEST):** UPSTREAM PORT WAVE 3 (RBAC depth) BAKED+committed + DEFAULT-ENABLE PASS.
- **Wave 3 = 6 upstream feats (0.0.412→432 port), all flag-OFF default, BAKED, committed on `feature/table-relevance-overview`, NOT pushed.** Mig chain now `svcacct1→usdquota1→fileref1` (**single head `fileref1`**). Round 1 (3 PARALLEL disjoint pods) + Round 2 (3 SERIAL, shared services+mig chain). Feats: #489 per-connection RBAC grants (`HYBRID_CONNECTION_GRANTS`, no mig, enables `connection` resource in permission_resolver/registry) · #488 USD monthly spend cap (`HYBRID_USD_QUOTA`, mig `usdquota1`, `usage_policies.monthly_spend_limit_usd`, enforce at `ai/llm/llm.py` fail-soft) · auto-publish agents (`HYBRID_AUTO_PUBLISH`, `_can_auto_publish_build` agent-admin tier, no mig) · #467 standalone tools-only connectors (`HYBRID_STANDALONE_CONNECTORS`, forces is_public=False + skips auto-agent hook, no mig) · #497 file references (`HYBRID_FILE_REFERENCES`, mig `fileref1`, `file_references` table→context builder inject, report→File) · #487 external MCP gateway (`HYBRID_MCP_GATEWAY`, `services/connection_tool_gateway.py` + 2 routes on existing `routes/mcp.py`, org-scoped fail-closed, no mig). 3 are API-only (FE deferred): standalone-connectors, file-refs, mcp-gateway. Detail → memory [[project_cityagent_upstream_port]]. W0+W1+W2+W3 all committed, NONE pushed.
- **DEFAULT-ENABLE PASS (user "enable all except risky"):** 140 UPGRADE_FLAGS (66 code-default-ON, 67 OFF, 7 env-only). Enabled **60** default-OFF flags via DB override on Main Org `b2bec83d` (`organization_settings.config.hybrid_overrides`, reversible, NOT code-default flip → keeps rule #4). **HELD 7** (dep/cost, still OFF): FULL_PIPELINE·DLT_INGEST·INGEST_BRAIN·PERSIST_WAREHOUSE·CROSS_SOURCE_UNIFY (ingest crash/numpy) · MOA·CODE_ENRICH_PLUS ($/query). **TAGGED `status="risky"`** (BAKED) 7 env-only: SKILLS·SUBAGENTS·SKILL_AUTOGROW·SKILL_OPTIMIZE·SKILL_OPTIMIZE_DAEMON·RECURSIVE·AUTOTRAIN_ON_INDEX. Live: **128 ON / 9 OFF** (`GET /api/organization/hybrid-flags`, parse `strict=False` — JSON has control chars). Alternatives for the 9 OFF: dlt→ROBUST_INGEST+ONE_TABLE_MERGE(on); full-pipeline→stages on piecemeal; cross-unify→ONE_TABLE_MERGE(on); autotrain→manual Train+Self-Learn(on); ingest-brain→ROBUST_INGEST(on), needs ghostscript+camelot bake for messy PDF/image; MOA→AUTO_MODEL(on); code-enrich-plus→CODE_ENRICH(on); studio-learn-daemon→per-studio Self-Learn(on). PENDING (advised, not done): hide 6 env-only knobs into HIDDEN_FLAGS (redundant Autotrain etc); bake F09 deps ONLY if ingesting scanned/messy files.
