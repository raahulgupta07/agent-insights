...
2. Per-user personal choice — lives in `localStorage.dash.locale` on the
...
4. System default — `dash_config.i18n.default_locale` (`en`).
...
frontend/plugins/i18n.ts        registers createI18n, applies personal
                                 dash.locale if present, exposes $setLocale
